 ...
 *
 * Applies scripts/out/propers-page-corrections.json to
 * src/data/loth/propers/*.json. Supports two target shapes:
 *   - targetField === 'page' : replace {node}.page (e.g. shortReading, responsory)
 *   - targetField === null   : replace {parent}[lastKey] (parallel key, e.g. intercessionsPage)
 *
 * Usage: node scripts/patch-propers-pages.js [--dry-run]
 */
...
const PATCH_FILE = path.join(ROOT, 'scripts/out/propers-page-corrections.json')
...
function applyEntry(data, entry) {
  ...
  // parallel key: last segment is the key on the parent
...
function main() {
  ...
  console.log(`=== patch-propers-pages ${dryRun ? '(dry-run)' : ''} ===`)
