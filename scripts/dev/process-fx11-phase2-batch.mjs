...
// F-X11 Phase 2-B (#442) — empty after detectRefrains generalisation
// (#435 — 2..4 dynamic longest-match-wins). Pre-#435 `EXCLUDE_REFS`
// pinned `Psalm 46:2-12` because the length=2-fixed extractor over-
// fragmented the 2-line refrain shape; the conservative shape was
// hand-curated by #417. With #435 the extractor now naturally produces
// stanza-relative PB [8,10,18,20] → builder coord [7,9,17,19] for
// Psalm 46:2-12 (verified live on book page 152 right column at #442
// dry-run), so the hotfix is now `algorithm-naturally-generated` and
// the EXCLUDE_REFS override can be dropped (SSOT consolidation per
// #442 dispatch + review-435 §8 권고). Pre-#434 `Psalm 80:2-8, 15-20`
// and `Psalm 8:2-10` were never in EXCLUDE_REFS — their hotfix was
// applied directly to rich.json by #434 and is similarly naturally
// generated by the new algorithm (verified by #436 review live PDF
// 6/6 spot-check). The Set is intentionally kept (not deleted) so
