...
describe('제2저녁기도 vs 다음날 제1저녁기도 (FR-175)', () => {
  ...
  describe('회귀 가드 — 기존 우선순위 규칙은 그대로', () => {
    ...
    it('평일 대축일 다음날이 평범한 주일이면 종전대로 주일 제1저녁기도가 이긴다', async () => {
      // 2026-08-15 토요일 성모 승천. 순위표상으로는 승천(I.3) > 연중 주일(II.6)
      // 이지만 카드와 본문이 이미 서로 일치(둘 다 주일 제1저녁기도)하므로
      // 이 변경의 범위 밖이다. 의도치 않게 바뀌지 않도록 고정한다.
      expect(cards('2026-08-15')).toEqual(['firstVespers', 'firstCompline', 'lauds'])
      const body = await assembleHour('2026-08-15', 'vespers')
      expect(body?.effectiveLiturgicalDay?.date).toBe('2026-08-16')
    })
