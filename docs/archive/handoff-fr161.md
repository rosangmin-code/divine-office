...
|---|---|---|---|---|
| FR-easter (audit) | #202 | docs/archive/handoff-fr-easter-regression.md (199 lines) | 9fa4ceb | 정적 분석상 resolver chain (propers-loader wk1 fallback + rich-overlay 3-tier + 5-layer merge + Layer5 Alleluia) 모두 정상. 정적 reproduction 안 됨 |
...
| FR-easter-2 (renderer audit) | #204 | docs/archive/handoff-fr-easter-regression.md §9 (94 lines 추가) | 36d933e | renderer 4 main propers 분기 정상. **★ Compline Marian seasonal default missing — never-implemented feature** 발견 (compline.ts L106 selectedIndex=0 하드코드) |
...
**핵심 audit 문서**: `docs/archive/handoff-fr-easter-regression.md` (commit 36d933e 기준 §1-§9 완성)
...
solver 가 audit 수행 — 3 suspects 모두 false positive 확정. week-*.json 변경 0건. 자세한 finding 은 `docs/archive/handoff-fr161-r14c.md` 참고.
...
- **C-2** (MEDIUM) — Lent Ave Regina 데이터 audit. **close**: PDF (`parsed_data/full_pdf.txt`) + 프로젝트 내 모든 데이터 (compline.json / common-prayers.json / gilh.json / ordinarium.json) sweep 결과 Ave Regina Caelorum 의 몽골어 번역 부재 확인. 현재 `compline.ts` 의 LENT → fallback idx=0 (Salve Regina) 동작이 PDF SSOT 한계를 정확히 반영. zero-data-change close. 추후 외부 자료 (몽골 가톨릭 별책 prayer book glossary 등) 가 import 되면 재오픈 가능.
- **C-3a** (LOW, wi-001 dev) — gospel canticle antiphonRich wiring (signature + assembler). `resolveGospelCanticle` 5번째 인자 추가, HourPropers/HourSection/RichOverlay Pick 모두 carry. land `157dc4e`. 7 files +61 -1, vitest +2 (284 PASS).
...
- **C-4** (LOW, wi-003 divine-researcher) — compline seasonal propers coverage audit. **close**: 5 propers/*.json (7,425 lines) sweep 결과 compline override 키 ZERO. Schema/loader/merger 모두 DayPropers.compline 지원하나 데이터 의도적 omit (Roman Rite 전통: compline = weekly cycle, 비-시즌). land `2716795`. audit doc `docs/archive/handoff-compline-c4-audit.md` (181 lines). peer codex AGREE r2 consensus.
...
**탐색 후보 (defer)**:
- F-1 / F-2 implementation (구현 우선순위 LOW-MEDIUM, 사용자 reported 회귀 0)
- F-3 / F-4 / F-5 (architectural cleanup, pre-existing)
...
- `docs/archive/fr-161-r7-pilot-psalm110-evidence.md` — Phase 0 milestone evidence
- `docs/archive/fr-161-r8-phase1-week1-evidence.md` — Phase 1 확산
- `docs/archive/fr-161-r9d-pattern-d-evidence.md` — multi-page mechanism
- `docs/archive/fr-161-r9a-pattern-a-evidence.md` — Daniel 44-rep canticle stress
- `docs/archive/fr-161-r9c-pattern-c-evidence.md` — section-title filter
- `docs/archive/fr-161-r9b-pattern-b-evidence.md` — defensive V/R
- `docs/archive/fr-161-r9e-typography-evidence.md` — typography normalize
- `docs/archive/fr-161-r10-evidence.md` — CI gate
- `docs/archive/fr-161-r12-1-column-merge-evidence.md` — extractor column boundary fix
- `docs/archive/fr-161-r12-3-deep-wrap-evidence.md` — depth raise (defensive)
- `docs/archive/fr-161-r14-pdf-indent-inconsistency.md` — 7-category audit (R-14)
...
- `docs/archive/handoff-fr161-r14c.md` — R-14c page-mapping audit findings (zero data change)
- `docs/archive/handoff-fr-easter-regression.md` — FR-easter audit (#202 §1-§8) + Priority C renderer audit (#204 §9). Compline Marian root cause + fix 권고
