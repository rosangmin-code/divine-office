...
### WI-A4 — 3 byte-equal verifier normalize 패치 (WI-B1 흡수, 사후 명문화)

> **이행 결과 (2026-04-26 commit 08ca495, task #89 = WI-B1 으로 흡수)**: 본 plan 작성 시점에는 별도 WI 로 식별되지 않았으나, WI-B1 회차에서 byte-equal verifier 3종 (`verify-first-vespers.js` / `verify-solemnity-first-vespers.js` / `verify-movable-first-vespers.js`) 의 `diff()` 함수에 `normalizeRef` on `.ref` paths 패치가 필수로 드러나 WI-B1 commit 에 함께 포함됨.

- **AC**:
  1. 3 verifier 의 `diff()` 함수가 `.ref` paths 에 한해 PDF extractor 의 bare-form 출력 (`Psalm 142`) 과 propers JSON 의 versed-form (`Psalm 142:1-7`) 을 동등 처리하는 `normalizeRef` 적용
  2. normalize 는 `parsed_data/first-vespers-versed-map.json` 의 `rewrite_needed=true` entries 에 한정 — Psalm 141:1-9 (rewrite_needed=false) 같은 already-versed entry 는 영향 없음
  3. 3 verifier 모두 mismatch=0 PASS — propers JSON 의 versed-form 과 PDF extractor 의 canonical bare-form 이 byte-equal 동등 (NFR-009c)
- **target_files**: `scripts/verify-first-vespers.js`, `scripts/verify-solemnity-first-vespers.js`, `scripts/verify-movable-first-vespers.js`
- **종속성**: WI-A1 (versed-map 데이터 소스)
- **complexity**: LOW
- **흡수 commit**: 08ca495 (WI-B1, task #89)

**정당화** (PDF extractor canonical bare ↔ propers versed-form 동등 처리):

- `verify-first-vespers.js` 류는 PDF "1 дүгээр Оройн даатгал залбирал" 섹션을 재추출해 propers JSON 과 byte-equal diff 로 drift 를 차단한다 (NFR-009c). PDF 본문이 명시하는 ref 는 bare 형 (`Psalm 142`) 이므로 extractor 도 bare 출력을 한다.
- 본 Phase 5 의 데이터 rewrite 결과 propers JSON 만 versed-form 으로 변경되며 PDF 자체는 무수정 — 따라서 patch 없이 verifier 를 돌리면 `expected="Psalm 142"` (bare) vs `actual="Psalm 142:1-7"` (versed) value-mismatch 가 시즌별 cell 수만큼 누적된다 (Phase 5 실측 — patch 없는 상태에서 lent 7 + easter 8 = 15건 silently mismatch). #90 머지 시점에는 이 사실이 발견되지 않아 main 의 verifier 가 잠시 회귀 상태였다.
- `normalizeRef` 는 `versed-map.json` 의 `rewrite_needed=true` entry 한정 — bare ↔ versed 양방향 동등 처리. WI-A1 의 단일 산출물이 SSOT 역할을 하므로 future drift 방지.
- 한번 적용하면 #90/#91/#94/#95 등 모든 후속 WI-B 시즌이 자동 수용 — WI 별 verifier 패치 중복 없음.

### WI-B1..B5 — 시즌별 적용 (5 시즌)

각 WI 형식 동일, 시즌 라벨만 변경:

- **WI-B1: easter** (8 cells) — task #89 (commit 08ca495). WI-A4 verifier normalize 패치 흡수.
- **WI-B2: lent** (7 cells) — task #90 (commit a44b9e4)
- **WI-B3: advent** (5 cells) — task #91 (commit cbf351c)
- **WI-B4: christmas + solemnities/12-25** (2 + 2 = 4 cells) — task #94 (commit f2fb22b)
- **WI-B5: ordinary-time** (42 cells) — task #95 (commit fd6db42)
...
### WI-D — Phase 5 cleanup + traceability + plan §4·§7 갱신

- **AC**:
  1. `verify-first-vespers-ref-coverage.js` 가 main branch 에서 exit 0 (모든 시즌 정합) — 0 violations
  2. `docs/PRD.md` FR-156 행에 Phase 5 항목 추가 (NFR-009b/d 준수 명시: bare→versed 데이터 normalization, byte-equal verifier 3종 mismatch=0)
  3. `docs/traceability-matrix.md` FR-156 영역에 Phase 5 WI 들 (WI-A1/A2/A3/A4/B1~B5/C/D) + 검증 스크립트 매핑 (`verify-first-vespers-ref-coverage.js`, `rewrite-first-vespers-bare-refs.js`, `extract-first-vespers-versed-map.js`) 등재
  4. `docs/archive/fr-156-phase5-firstvespers-bare-ref-plan.md` §4 갱신: WI-A4 신설 (verifier 3종 normalize 패치, WI-B1 흡수) + WI-B1..B5 의 task ID/commit 추가. §7 갱신: Open Question Q2 (parser regex bare-ref 지원) 결정 명문화 — Phase 5 에서는 **무수정** (verifier-gated 데이터 normalization 으로 충분) 으로 ADR-style 기록
  5. **parser regex 평가**: `src/lib/scripture-ref-parser.ts` 무수정 — verifier-gated 데이터 normalization 으로 충분 (R-1 별건 FR 보류). 본 PR 의 §7 갱신에서 결정 명시
- **target_files**: `docs/PRD.md`, `docs/traceability-matrix.md`, `docs/archive/fr-156-phase5-firstvespers-bare-ref-plan.md`
- **종속성**: WI-B1..B5 모두 완료 (#89/#90/#91/#94/#95)
- **complexity**: LOW
- **suggested_skill**: `/pair-perfect --adhoc`
- **isolation**: worktree
...
### 7.2 Open Questions (leader 결정 — Phase 5 종료 시점 갱신)

> **갱신 (2026-04-26 / WI-D, task #96)**: Phase 5 머지 후 회고로 Q1~Q6 모두 결정 완료.
...
### 7.3 Phase 5 후속 결정 ADR (2026-04-26)
