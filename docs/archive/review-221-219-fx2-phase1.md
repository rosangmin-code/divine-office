...
- `git diff e7973f37..0471b05` 에 `docs/archive/handoff-fx2-psalmprayer-audit.md` 가 deletion (-382) 으로 표시. 0471b05 의 부모가 `ca009bc` 이며 audit doc 은 `161f247` 에서 추가되어 base merge `e7973f37` 에 포함됐기 때문. leader merge 시 `worktree-219-member-01` 을 `e7973f37` 위로 ff-merge 하면 audit 가 그대로 보존됨. **실제 삭제 아님**.
