...
> **정책 정정 (2026-05-14)** — 시편 본문 (psalm body, `data-role="psalm-stanza"` 범위) 안에서는 refrain 을 포함한 전체 텍스트가 까만색으로 통일된다. 사용자 directive: "시편 본문에서는 모두 까만 글씨로 들어가야 해. 전수 조사에서 수정해 줘." `psalm-stanza-refrain` / `psalm-phrase-refrain` data-role 메타데이터는 보존 (회중 응답 식별 + denylist/allowlist 정합화 (FR-160) + e2e selector). §12.1 의 다른 빨간색 루브릭 (FR-126 dismissal, FR-127 invitatory rubric, FR-128 Gloria Patri 생략, FR-150 intercessions dash, FR-152 responsory dash, FR-160-A1/A4 denylist/allowlist 데이터 모델, FR-160-B conditional rubric directive, FR-160-C psalter-headers preface) 는 본 정책 정정에 영향받지 않는다 — 시편 본문 외 영역의 루브릭 색상 contract 는 그대로 유지.
...
| FR-160-B | **Inline conditional + page-redirect 데이터 모델 (PR-1: schema + Layer 4.5 hydrate)**: PDF 원전 ~150–170 cells 의 (1) 조건부 지시문 (~80–100, "X 의 자리에 Y 를 읽는다" / "송영을 생략한다") 과 (2) 페이지 리다이렉트 (~70, "Магтуу: х. 879") 를 일반 표현하는 데이터 모델 신설. plan: `docs/fr-160-phase-b-conditional-redirect-plan.md` (peer R4 AGREE, 502 lines). **PR-1 (task #139, 완료) — B1+B2 통합 PR**: schema + types + Layer 4.5 hydrate. `src/lib/types.ts` 에 5 신규 type — `ConditionalRubric` (rubricId/when/action/target/appliesTo/evidencePdf/liturgicalBasis) + `ConditionalRubricAction` enum (skip/substitute/prepend/append) + `ConditionalRubricSection` enum 10종 + `ConditionalRubricWhen` (season/dayOfWeek/dateRange MM-DD/predicate) + `PageRedirect` (redirectId/ordinariumKey/page/label/appliesAt/evidencePdf) + `PageRedirectOrdinariumKey` 닫힌 enum 9종 (benedictus/magnificat/nunc-dimittis/dismissal-blessing/compline-responsory/common-prayers/gloria-patri/invitatory-psalms/hymns). `HourPropers` 에 optional `conditionalRubrics?: ConditionalRubric[]` + `pageRedirects?: PageRedirect[]` 필드 추가 (additive, 기존 데이터 미터치). `src/lib/schemas.ts` 에 Zod runtime — `ConditionalRubricSchema` (action='skip' 외 target 강제 refinement) + `PageRedirectSchema` (page 1..969 + closed-enum ordinariumKey) + `OrdinariumKeyCatalogSchema`. 신규 catalog `src/data/loth/ordinarium-key-catalog.json` (9 keys → 표준 PDF 페이지/sourcePath). Layer 4.5 hydrate 모듈 — `src/lib/hours/conditional-rubric-resolver.ts::applyConditionalRubrics(propers, ctx)` (`matchesWhen` 평면 매처 + skip/substitute/prepend/append 4 action 적용; immutable shallow-copy 계약) + `src/lib/hours/page-redirect-resolver.ts::applyPageRedirects(propers, catalog)` (catalog 부재 ordinariumKey fail-hard, mtime cache loader). `src/lib/loth-service.ts` Layer 4 직후 hydrate 호출 추가 — pageRedirects 가 빈 배열이거나 부재 시 catalog 로드도 skip (zero-cost noop). 신규 verifier — `scripts/verify-conditional-rubrics.js` + `scripts/verify-page-redirects.js` 가 5 시즌 propers + 4 sanctoral JSON 의 신 필드를 zod parse + 카탈로그 무결성 검증 (CI gate 후보, NFR-009i 신설 예상). 단위 — schemas.test.ts 에 11 신규 case (positive/negative/closed-enum/refinement) + conditional-rubric-resolver.test.ts 12 case (when 매처 + 4 action 적용 + immutable + noop) + page-redirect-resolver.test.ts 8 case (catalog load + cache + fail-hard + PR-1 byte-equal). vitest 332 → **400 PASS**, tsc 0, sw.js 무수정 (AC-9 PASS), 4 기존 verifier (psalter/hymn/compline/propers/psalter-body/sanctoral) 회귀 0. **PR-2~7 (B3, member-01, 완료)**: 시즌별 1차 마킹 — advent/christmas/lent/easter/ordinary-time + sanctoral 6 sub-PR (#145~#150). 12 ConditionalRubric + 40 PageRedirect entries 데이터 주입 — psalmody substitute 9 (Easter Sunday + Pentecost Lauds/Vespers2 + sanctoral memorial 11-02 All Souls Sun 등) + hymn substitute 3 (sanctoral feasts) + invitatory pageRedirect 24 (advent/christmas/easter/lent + sanctoral solemnities/feasts) + dismissal pageRedirect 16 (5 시즌 advent/christmas/easter/lent). **PR-8 (B4, task #151, 완료)**: applyConditionalRubrics 의 PR-1 미커버 5 sections × 4 actions 확장 — psalmody/intercessions(substitute/prepend/append)/invitatory/dismissal/openingVersicle. PR-1 의 substitute/prepend/append 가 concludingPrayer/hymn/shortReading 에만 적용되던 한계 해소 (PR-7 R2 지적 11-02 substitute psalmody runtime gap 해결). 산출 — `src/lib/types.ts`: 신규 `SectionOverride` interface (rubricId/mode/text?/ref?/ordinariumKey?) + `SectionOverrideMap` (`Partial<Record<ConditionalRubricSection, SectionOverride[]>>`) + `HourPropers.sectionOverrides?` optional 필드 (additive). `src/lib/hours/conditional-rubric-resolver.ts`: 5 신규 sections 처리 — 4 sections (psalmody/invitatory/dismissal/openingVersicle) 는 `appendSectionOverride()` 로 `sectionOverrides[section]` 에 SectionOverride 누적 (printed body 가 ordinarium/psalter/sanctoral.properPsalmody 에 있어 HourPropers 외부; 어셈블러/UI 가 surface 결정), intercessions 는 actual array 도 변경 (substitute → `[text]`, prepend → unshift, append → push, skip → delete) + override 도 누적 (audit/evidence). target.text 부재 (ref/ordinariumKey-only) substitute 도 sectionOverride 에 ref/ordinariumKey hint 만 기록. PR-1 immutable shallow-copy 계약 보존. 단위 — conditional-rubric-resolver.test.ts +18 case (11-02 substitute psalmody / wrong dayOfWeek noop / skip-substitute-prepend-append × 5 신규 sections / multiple PR-8 rubrics 누적 / pre-PR-1 sections regression / dispatch matrix EASTER+isFirstHourOfDay / immutable). vitest 402 → **420 PASS** (+18), tsc 0, sw.js 무수정 (AC-9), 6 page verifier 회귀 0, 3 신규 verifier OK (12 CR + 40 PR validated), audit-psalter-key-dedup --check OK. UI 렌더링 (PR-9 B5): assembler 가 sectionOverrides 를 읽어 psalmody/invitatory/dismissal/openingVersicle 위에 directive notice 표시 + intercessions 는 mutated array 그대로 렌더. **PR-9a (UI 렌더링 surface, task #156, 완료)**: PR-8 의 sectionOverrides 가 client UI 까지 전달되도록 5 HourSection variants (psalmody/intercessions/invitatory/dismissal/openingVersicle) 에 optional `directives?: SectionOverride[]` 필드 추가 + assembler 헬퍼 `attachSectionDirectives(section, mergedPropers)` 신설 (`src/lib/hours/shared.ts`) — lauds/vespers/compline 의 5 section build 시점에 mergedPropers.sectionOverrides[section.type] 을 section 으로 propagate. 신규 reusable `<DirectiveBlock directives, filterMode>` 컴포넌트 (`src/components/prayer-sections/directive-block.tsx`) — `data-role="conditional-rubric-directive"` + `data-mode={mode}` + `data-rubric-id` 속성 부여 (PR-9b e2e selector 약속), text 부재 시 ref / ordinariumKey hint 로 fallback 표시 + `partitionDirectives()` 헬퍼 (mode 별 분기). 5 section 컴포넌트 mode 별 surface — skip(without substitute)/substitute = body 숨김 + directive 표시, prepend = directive 앞 + body, append = body + directive 뒤. 11-02 sanctoral substitute psalmody 가 Sunday 에 fire 시 PsalmodySection 이 psalms 대신 rubric directive 를 빨간 italic 으로 surface (body hidden). intercessions 는 PR-8 array mutation + PR-9a directive notice dual surface (audit/clarity). 신규 단위 — `section-directives.test.ts` 12 case (assembler attachSectionDirectives helper: PR-8 5 sections propagate / non-PR-8 sections passthrough / 부재·빈 배열 noop / multiple directives ordering / immutability) + `directive-block.test.ts` 18 case (helper partitionDirectives 2 + DirectiveBlock 5 + PsalmodySection 5 [substitute=11-02 / skip / prepend / append / regression] + DismissalSection 3 + IntercessionsSection 3) — react-dom/server.renderToStaticMarkup 으로 jsdom 의존 없이 렌더 검증. vitest 423 → **453 PASS** (+30), tsc 0, sw.js 무수정 (UI 렌더링은 navigation 정책 무관 — 새 route/Content-Type 0, AC-9), 6 page verifier 회귀 0, 3 신규 verifier OK (12 CR + 40 PR + dedup), audit-psalter-key-dedup --check OK. 모바일 스크린샷 (`scripts/capture-fr160b-pr9a-screenshots.mjs`, Pixel 7 Playwright, `.playwright-mcp/fr160b-pr9a-{easter-sunday,pentecost}-lauds-psalmody-substitute.png`) — 2026-04-05 Easter Sunday Lauds + 2026-05-24 Pentecost Lauds 각각의 psalmody substitute directive 가 Mongolian Cyrillic ("Дууллууд ба магтаалыг 1 дүгээр долоо хоногийн Ням гарагаас татаж авна. х. 58.") 빨간 italic 으로 정상 surface 확인. **PR-9b (B5, task #161, 완료)**: e2e 4 spec + dispatch matrix vitest. PR-9a 의 `data-role="conditional-rubric-directive"` + `data-mode` + `data-rubric-id` 안정 selector 가 유지되는지 12 ConditionalRubric × 5 시즌 + 40 PageRedirect catalog 정합 양면 검증. **신규 e2e 26 cases 4 spec** — `e2e/conditional-rubric-easter.spec.ts` 5 (Easter Sunday lauds substitute / Pentecost lauds substitute / Pentecost vespers2 cell 데이터 구조 보존 / Easter weekday negative / Easter Sunday vespers cell-scope negative), `e2e/conditional-rubric-advent.spec.ts` 4 (dec24 dateRange match / dec23 dateRange miss before-from / dec04 wide miss / dec24 vespers cell-scope negative), `e2e/conditional-rubric-sanctoral.spec.ts` 11 (5 active dispatch: 11-02 SUN lauds substitute / 11-02 SUN vespers substitute / 11-02 MON dayOfWeek miss / Christmas dec25 lauds substitute / dec25 vespers cell-scope negative + 5 data-layer authored coverage: OT W1 Baptism + OT W34 Christ-the-King + 02-02 Presentation hymn + 08-06 Transfiguration hymn + 09-14 Holy Cross hymn — 5 rubrics on calendar-pre-empted or non-DIRECTIVE-sections [hymn ∉ DIRECTIVE_SECTION_TYPES] + 1 inventory invariant: 12 ConditionalRubric 전체 closure check), `e2e/page-redirect.spec.ts` 6 (catalog 9 키 schema / 40 PageRedirects fixed-kind page byte-equal validation / Pentecost lauds 2-redirect render / dismissal-blessing variable-kind 다중 시즌 확산 / Pentecost dismissal section body hydrated from common-prayers ordinarium catalog sourcePath / Mongolian Cyrillic 라벨 NFR-002). **신규 dispatch matrix vitest** — `src/lib/__tests__/hours/dispatch-matrix.test.ts` 111 cases (5 seasons × 7 days = 35 cells × 3 dispatch tests [match positive / season axis flip negative / dayOfWeek axis flip negative] + 3 action enum coverage [skip/prepend/append] + 2 predicate axis [isFirstHourOfDay lauds match / vespers miss] + 1 cell count assertion). vitest 459 → **570 PASS** (+111), tsc 0, sw.js 무수정 (e2e 검증만, navigation 정책 무관 AC-9), 6 page verifier 회귀 0, 3 신규 verifier OK (12 CR + 40 PR + dedup --check), `@fr FR-160-B-5b` 태그 21 식별자 모두 traceability matrix 등록. AC-4 (PageRedirect hydrate 본문 byte-equal) 는 PARTIALLY_MET — peer R2 가 정확히 지적한 바, `buildDismissal` 이 PageRedirect 와 무관하게 common-prayers.json 본문을 assemble 하므로 catalog cross-validation (ordinariumKey 닫힌 enum 9 + fixed-kind page byte-equal + variable-kind 다중 시즌 span) + Pentecost dismissal smoke (section body 정상 어셈블 확인) 까지만 검증. 실제 PageRedirect 가 body 를 inject/replace 하는 hydrate 단계는 후속 PR-10 에서 처리. **PR-10 (B6, task #162, 완료)**: PageRedirect inline body hydrate — PR-9b AC-4 PARTIALLY_MET 닫기. `applyPageRedirects` 가 catalog `sourcePath` (e.g. `canticles.json#benedictus`, `common-prayers.json#dismissal`) 의 본문을 mtime-cache 로 로드해 `HourPropers.pageRedirectBodies?: HydratedPageRedirect[]` (internal full body) 에 부착, `loth-service.ts` 가 `AssembledHour.pageRedirectBodies?: PageRedirectBodyMeta[]` 로 metadata-only mirror (peer R1 우려 — `hymns` 134KB body API surface bloat 차단, body 는 internal 에만 retain). 신규 type — `HydratedPageRedirect` (internal: redirectId/ordinariumKey/page/label/appliesAt/catalog{kind,page,label,sourcePath}/body) + `PageRedirectBodyMeta` (public: same fields without body). 신규 helper — `parseSourcePath()` (`file#dot.path` → `{file, pointer[]}`) + `resolvePointer()` (fail-hard on missing segment) + `hydrateRedirectBody()` (단일 redirect 처리, sourcePath 부재 fail-hard) + `loadOrdinariumFile()` (mtime-aware Map cache). 9 ordinariumKey × sourcePath fail-hard gate — catalog drift 가 runtime 으로 새지 않도록 build-time verifier `scripts/verify-page-redirect-bodies.js` 신규 (9/9 hydrate clean). 단위 — page-redirect-resolver.test.ts +16 case (9 keys × byte-equal + 3 fail-hard negative [missing sourcePath / unresolved pointer / missing file] + 4 integration [propers attach / preserve existing fields / referential equality on noop / determinism]). e2e — page-redirect.spec.ts AC-4 byte-equal MET 업그레이드 (catalog.sourcePath 기반 cross-validation + body absent assertion + dismissal section.priest deep-equal common-prayers.json#dismissal.priest) + no-redirect 회귀 가드 (OT 평일 → pageRedirectBodies undefined). vitest 570 → **586 PASS** (+16), tsc 0, e2e/page-redirect 14 PASS / 0 FAIL (10 기존 + 2 신규 + R1 fix-up 헬퍼 변경), e2e/conditional-rubric-{easter,advent,sanctoral}+api 62 PASS / 0 FAIL, sw.js 무수정 (AC-9), 6 page verifier 회귀 0, 4 신규 verifier OK (12 CR + 40 PR + dedup --check + 9/9 PRB hydrate). API live probe 2026-05-24 lauds → `pageRedirectBodies[]` 2 entries (invitatory-psalms + dismissal-blessing) metadata-only (body 없음); 2026-02-04 lauds → 필드 omit. peer codex code_reviewer R1 APPROVED_WITH_ISSUES (MED) → R2 fix-up (PageRedirectBodyMeta + body strip) → R3 AGREE consensus, dec_1. AC 매핑: AC-1 (9 keys hydrate) ✓ unit + verifier, AC-2 (byte-equal) ✓ unit per-key + e2e dismissal deep-equal, AC-3 (no regression) ✓ noop referential equality + git stash baseline 동일 결과, AC-4 (assembleHour 결정성) ✓ 2회 호출 deep-equal unit, AC-5 (≥9 unit) ✓ 16 신규, AC-6 (e2e AC-4 PARTIALLY_MET → MET) ✓ byte-equal upgrade 명시, AC-7 (FR-160-B-6 태그) ✓ 모든 신규 case. | 데이터 / 파이프라인 / UI | P1 | PR-1~PR-10 완료 (schema + Layer 4.5 hydrate + 6 시즌 1차 마킹 + 5 신규 sections × 4 actions + 11-02 runtime gap 해소 + UI surface + e2e 26 cases + dispatch matrix 111 cases + ordinarium body inline hydrate + AC-4 MET). FR-160 Phase B 마무리 |
| FR-160-B-6 | **이동 대축일 substitute-psalmody 실시편 인라인 해소 (logic-resolve, GOAL #13)**: 이동 대축일의 "Дууллууд ба магтаалыг 1 дүгээр долоо хоногийн Ням гарагаас татаж авна. х. 58" psalmody-substitute 루브릭이 안내문(pointer-note)만 보이던 버그를 해소 — 실제 제1주간 시편 본문 + 후렴을 인라인 렌더한다. `ConditionalRubricTarget` 에 구조화 `psalterRef?: {week,day,hour}` 필드 신설(`src/lib/types.ts` + `src/lib/schemas.ts` Zod `PsalterRefSchema`). `src/lib/hours/conditional-rubric-resolver.ts::resolvePsalmodySubstituteRef(sources, ctx)` 가 매칭 substitute-psalmody 루브릭의 psalterRef 를 반환하고, `src/lib/loth-service.ts` step 6.5(시편 텍스트 resolve 직전)이 `getPsalterPsalmody(week,day,hour)` 로 빌린 시편을 `psalmEntries` 에 주입 → 기존 `resolvePsalm` 절기 후렴 경로 재사용(기존 `antiphons` override map → `antiphonOverrides` 우선; 없을 때만 default + EASTER Alleluia 증대 GILH §113). **D3 정합성(#15 감사 반영)**: 미사경본 substitute 규칙은 시편 *본문*만 Week-1 SUN 에서 차용하고 **후렴은 대축일 고유**다 — 따라서 빌린 Week-1 default 후렴을 그대로 쓰면 틀린다. 각 셀에 대축일 고유 후렴(미사경본 PDF 발췌)을 `antiphons` map(빌린 시편 antiphon_key `w1-sun-lauds-ps1/cant/ps3` · `w1-sun-vesp-ps1/ps2/cant` 키잉)으로 주입: Easter Sunday lauds(p.690) / Pentecost lauds(p.739) + vespers2(p.742) / Christmas lauds(p.589). 평일 default 후렴 오적용 없음. resolver 가 substitute SectionOverride 에 `bodyInlined: true` 마킹(psalterRef 보유 시에만) → `src/components/prayer-sections/psalmody-section.tsx` 는 `bodyInlined` substitute 일 때만 본문 시편을 렌더 + 안내문을 작은 affordance 로 보존하고, `bodyInlined` 없는 substitute(Type A'/All Souls)·skip 은 기존대로 본문 숨김(안내문 단독) — **A'/B 무회귀 보장**(psalms 부재 시도 방어적으로 안내문 단독). 적용 셀(bodyInlined·렌더): easter(EasterSunday/Pentecost lauds + Pentecost vespers2 latent) + christmas(dec25 lauds) → psalterRef week1 SUN + proper antiphons. 비대상(현행 안내문 유지): advent(dec24 lauds, 동적 Type A' "진행 중 주간" — psalterRef 미부여, proper 후렴 주입은 동적 시편슬롯 키잉 필요해 GOAL #20 후속)·All Souls(11-02 Sun)·ordinary-time wk1/wk34 SUN(day-level calendar-redirect, hour-cell 미도달) — resolver 는 psalterRef 존재 시에만 inline 하므로 Week-1 강제 인라인 없음. ROOT CAUSE: Pentecost 가 psalterWeek 4 의 잘못된 시편을 로드한 뒤 UI 가 숨김(이중 버그). **GOAL #20 (#20-sub-2, 완료) — 시편 부재 5 이동 대축일 확장**: FR-160-B-6 substitute 메커니즘을 고유 시편이 인쇄되지 않은 5 이동 대축일(Trinity / Corpus Christi / Sacred Heart / Christ the King / Ascension)의 **Lauds + 2저녁기도(vespers2)** 로 확장. 4주 시편집(`four-week-psalter.txt`) 해당 섹션을 직접 정독해 고유 시편 미인쇄(Магнификат/Бенедиктус 후렴 + 기도만 인쇄) 확인 → Week-1 SUN 차용(GILH 일반규범, 부활/성령강림 'х.58' 선례). 각 셀에 발췌 Бенедиктус(Lauds)/Магнификат(2저녁) 후렴 + `conditionalRubrics` psalterRef(`week1/SUN/{lauds,vespers}`) 주입. FR-160-B-6 D3 와 달리 이 5 대축일은 **고유 시편후렴이 인쇄되지 않으므로** Week-1 default 후렴 사용(`antiphons` override 없음) — 추측 번역 0건. 평일 대축일(Sacred Heart 금 / Ascension 목)은 vespers2 substitute 의 `when` 을 dayOfWeek:SUN 게이트 없는 season-only 로 두어 SUN-슬롯 블록 fallback 으로 발동. 부수로 firstVespers prayer 의 runaway 파싱오염 6셀 절단(easter pentecost/ascension altPrayer + OT trinity/corpus/sacredHeart altPrayer + christTheKing concludingPrayer) + 'Эзэний баяр' 페이지머리글 잡음 제거(삼켜졌던 2저녁 Магнификат 후렴은 위 vespers2 구조화 주입으로 무손실 복원). 검증: `movable-solemnity-vespers2.test.ts` 17 it(L2 assembleHour 실측) + `e2e/conditional-rubric-movable-solemnity.spec.ts` 12. `@fr FR-160-B-6`. | UI/데이터/로직 | P1 | 완료 (+ GOAL #20 5 이동 대축일 확장) |
| FR-160-B-7 | **위령의 날(11-02) 일요일겹침 시편 동적 인라인 (GOAL #27 / #27-sub-2)**: 위령의 날이 일요일과 겹치는 해(예: 2025-11-02, 2031-11-02 = 연중 제31주일)에 Lauds·Vespers psalmody substitute 루브릭(`when.dayOfWeek=[SUN]`)이 "Дөрвөн долоо хоног 의 해당 일요일에서 가져오라"는 **안내문만** 띄우고 실제 시편 본문을 숨기던 문제 해소. FR-160-B-6 의 `psalterRef` 메커니즘을 **동적 주차**로 확장 — `psalterRef.week` 에 `'current'` sentinel 추가(`src/lib/types.ts` + `src/lib/schemas.ts::PsalterRefSchema` z.literal('current')). `src/lib/loth-service.ts` step 6.5 가 `'current'` → 그날의 `day.psalterWeek`(4주 시편집 cycle 의 "тохиож буй зөв Ням гараг", PDF p.839 = 진행 중 주차의 일요일) 로 narrow 해 `getPsalterPsalmody(week, SUN, hour)` 본문을 인라인. memorials.json 11-02 lauds·vespers substitute target 에 `psalterRef:{week:'current',day:'SUN',hour}` 주입(신규 rubric 없음·기존 2 루브릭 확장이라 inventory 22 불변). 후렴 보존(기존 gospelCanticleAntiphon 무수정). **회귀 0 게이트**: `ConditionalRubricContext.isEveOfFollowingDay`(= `effectiveDayOfWeek !== dayOfWeek || isFirstVespers || isFirstCompline`) 신규 — Saturday-eve 11-02(토요일 저녁 = 다음 일요일 First Vespers, SUN 승격으로만 매칭)에서는 동적 borrow 와 `bodyInlined` 둘 다 억제해 기존 안내문-only surface 유지(2024/2030-11-02). 평일(월~금) 11-02 는 substitute 미발동(無변경). 고정-week psalterRef(부활/성령강림/성탄 Week-1)는 eve-gate 무영향. SW CACHE_VERSION bump 불요(JSON 번들 청크·자산경로/Content-Type/라우트 무변경, navigation network-only). | UI/데이터/로직 | P1 | 완료 — true-Sunday 본문 인라인 + Saturday-eve·평일 회귀 0 (vitest 1492 PASS, e2e conditional-rubric 38 PASS) |
| FR-161 | **시편/기도문 phrase-unit-aware 줄바꿈 (architectural pivot)**: PDF 의 visual line break 를 1:1 로 web 에 옮기던 정책 (FR-153f stanza 라인 단위 `<span class="block">` 렌더) 폐기. PDF typography 의 phrase-end (flush left) ↔ phrase-wrap (indented continuation) cue 로 **phrase 단위를 추출**해 schema 에 명시하고, renderer 가 phrase 단위로 `<p>` emit (verse line break 는 viewport-aware soft wrap 으로 격하). 사용자 발화 (2026-04-29 — "PDF 줄바꿈을 고수하려고 하니 이중으로 줄바꿈이 들어가서 엉망이 되어버린 거지") 응답. **plan**: `docs/fr-161-phrase-unit-pivot-plan.md` (557 lines, planer task #172, leader cherry-pick HEAD `aa88e09`). Sub-WI R-0~R-12 (12개): R-0 PDF 재파싱 spike (의존 root), R-1 extractor (`scripts/parsers/extract-phrases-from-pdf.mjs`), R-2 builder (`scripts/build-phrases-into-rich.mjs`), R-3 schema (`PrayerBlock.kind:'stanza'.phrases?: PhraseGroup[]`, `src/lib/types.ts` + Zod `src/lib/schemas.ts`), R-4 renderer (`src/components/psalm-block.tsx`, phrases 분기 vs legacy line-render fallback), R-5 rich-content stanza 분기 (`src/components/prayer-sections/rich-content.tsx`), R-6 verifier (`scripts/verify-phrase-coverage.js`, NFR-009j 신설), R-7 pilot Psalm 110:1-5,7 (Sun Vespers I, end-to-end milestone), R-8 Phase 1 (week 1 ~30 시편), R-9 Phase 2 (w2-w4 + canticles + invitatory + compline ~95 ref), R-10 기도문 stanza-kind, R-11 PRD/traceability (본 행), R-12 Phase 3 optional (legacy line-render 폐기). **R-0 (task #173, 완료)**: `pdftotext -layout` 채택 + pdfminer.six XML fallback (가정 위배 0, plan §13 가정 1 검증). 산출 `docs/fr-161-r0-pdf-reparse-spike.md`. **R-1~R-6 (tasks #175~#178, 완료)**: extractor + builder + schema + renderer + verifier 일체. 6 sub-WI 분담 (member-01 R-1/R-2/R-3/R-4, divine-tester R-6). NFR-009j 신설 (phrase coverage 4 invariant — schema 통과 / bounds / non-overlap / gap-less coverage). **R-7 (task #179, 완료) — Phase 0 milestone**: pilot Psalm 110:1-5, 7 (Sun Vespers I, week-1.json:98) end-to-end 4-stage pipeline (extract → reconcile → inject → verify) PASS. Reconciliation 결정 — option (b) "rich.json pre-joined wrap pair 를 PDF-visual 2 line 으로 분리" 채택 (R-8+ scale-up canonical pattern). 산출 `docs/fr-161-r7-pilot-psalm110-evidence.md` (276 lines). **R-8 (task #180, 완료)**: Phase 1 week 1 batch — 42 refs scan, **24 refs (57%) injected** (1 R-7 pilot + 23 신규, auto-applied 48 wrap-line splits across 18 refs), 18 refs (43%) deferred to R-9 as 4 novel-edge patterns. R-6 verifier 0 violations 44 stanzas. 산출 `docs/fr-161-r8-phase1-week1-evidence.md` (302 lines) + `scripts/dev/process-week-phrases.mjs` + `scripts/dev/auto-reconcile-wraps.mjs`. **R-9~R-12 (후속)**: Phase 2 (~95 ref multi-PR LARGE) + 기도문 stanza-kind + Phase 3 legacy 폐기 (optional, 안정화 후). **마이그레이션 진행 표 (2026-05-29 현행화)**: Phase 0 ✓ (R-7 milestone) / Phase 1 ✓ (R-8) / Phase 2 ✓ (R-9~R-19 typography·sentence-mode·fx3 method-b spike + F-X11 문단-break audit Phase 2 A~K 100% closure + #499~#503 sweep 122 refs paragraphBoundaries inject) — `verify-phrase-coverage --check` 354 stanza 0 violations PASS / Phase 3 (legacy line-render 폐기) optional 미착수. **SW 무수정** — navigation network-only (CLAUDE.md), CACHE_VERSION bump 불필요. **결정 게이트 D-1~D-4** (schema option / FR 번호 / pilot 시편 / phase 분할) 모두 사용자 결정 완료 — Option B / FR-161 / Psalm 110 / 3-phase. NFR-009j 신설 + 본 PRD/traceability-matrix 등재 (R-11). | 데이터 / 파이프라인 / UI | P1 | Phase 0 (R-0~R-7) + Phase 1 (R-8) + R-9~R-19 (typography drift handler / sentence-mode flow / fx3 method-b spike) + F-X11 문단-break audit (Phase 2 A~K 100% closure) + #499~#503 paragraph/phrase sweep (122 refs paragraphBoundaries inject) 모두 land. `node scripts/verify-phrase-coverage.js --check` = 354 stanza 0 violations PASS. NFR-009j 신설. 잔여: 전 corpus 완전 커버리지 audit + Phase 3 legacy line-render 폐기(optional). |
| FR-156 | **First Vespers of Sunday 전용 propers 지원**: 현재 앱의 Saturday vespers 는 `loth-service.ts` 의 날짜 fallback 으로 다음 Sunday 의 regular vespers propers 를 재사용한다 (line 94-99). 그러나 PDF 에는 각 Sunday 마다 별도 "1 дүгээр Оройн даатгал залбирал" (First Vespers of Sunday) 섹션이 존재 — 고유한 psalm 배열 + 4-5종 seasonal variant (default / advent / lentSunday[N] / lentPassionSunday / easterSunday[N]) 를 가진다. 예: Week 2 Sunday 1st Vespers (PDF p166-167) Psalm 119:105-112 + 두 시편. **현재 한계**: FR-155 변형 게이트는 psalter 의 default_antiphon 과 PDF 앵커 prefix 매치를 전제로 작동하는데, First Vespers 의 default_antiphon 은 psalter 계통과 다름 → task #18 에서 3건 (w2 first vespers Passion Sunday marker, PDF line 5554/5618/5693) 이 orphan 으로 drop 됨. 해결 방향: 스키마에 `FirstVespersPropers` 타입 + `propers/{season}.json` 의 각 Sunday 에 `firstVespers` 섹션 + `loth-service.ts` 가 Saturday vespers 에서 Sunday regular 가 아닌 First Vespers propers 를 우선 채택하도록 분기. 구현 시 Phase 분할 권장: 스키마+resolver / 데이터 추출 / 렌더 / 테스트. 범위 MEDIUM~HIGH (~4일 추정). 상세 스코프는 `docs/fr-156-first-vespers-scope.md`. **Phase 1 (task #19, 완료)**: 스키마 + resolver wiring. `src/lib/types.ts` 에 `FirstVespersPropers` (HourPropers extends + optional own `psalms?: PsalmEntry[]`) + `DayPropers.firstVespers?` 추가. `src/lib/propers-loader.ts` 에 `getSeasonFirstVespers(season, sundayWeekOfSeason, dateStr?, celebrationName?)` 추가 — date-key / Easter-special key / 주간 key 3층 lookup (기존 `getSeasonHourPropers` 와 동일 패턴). `src/lib/loth-service.ts` 의 Saturday-vespers 분기 확장: 다음 Sunday 의 `firstVespers` 존재 시 우선 채택 + `firstVespers.psalms` 존재 시 `psalmEntries` 대체, 부재 시 기존 Sunday regular vespers fallback 유지. `schemas.ts` 는 변경 불필요 (`SeasonPropersFileSchema` 가 `z.record(z.string(), z.unknown())` 로 이미 permissive). 신규 unit `first-vespers.test.ts` 3 케이스 — (a) 전 시즌/주차에 대해 현재 `getSeasonFirstVespers` 가 `null` 반환 (Phase 2 데이터 주입 전 상태), (b) mock 된 firstVespers 가 regular vespers 를 덮어쓰는지, (c) firstVespers 부재 시 기존 regular vespers fallback 유지 (회귀 가드). 총 `npx vitest run` 244 PASS (+3) / `npx tsc --noEmit` 0. **Phase 2 (task #20, 완료, commit 9da194f)**: PDF "1 дүгээр Оройн даатгал залбирал" 섹션 추출 스크립트 (`extract-first-vespers.js`) + `propers/{season}.json` 데이터 주입 + verifier. 현 스키마 그대로 runtime 에 반영 완료. **Phase 3a (task #21, 완료)**: Solemnity First Vespers 스키마 + resolver wiring. `SanctoralEntry.firstVespers?: FirstVespersPropers` 신규 필드 (기존 `vespers2` 와 대칭 — `vespers2` 가 solemnity 당일 2nd Vespers 를 다루는 반면 `firstVespers` 는 전날 저녁 1st Vespers 를 다룸). `src/lib/loth-service.ts` 의 vespers 분기에 "evening-before-solemnity" 블록 추가 — `getLiturgicalDay(tomorrow)` 가 SOLEMNITY 이고 그 날의 `sanctoral.firstVespers` 존재 시 **최상위 우선순위**로 채택 (기존 seasonPropers / Saturday→Sunday firstVespers 를 displace). 우선순위 체계: **Solemnity First Vespers > Sunday First Vespers > Sunday regular vespers fallback**. solemnity 데이터는 Phase 3b 에서 주입되므로 Phase 3a 시점 분기는 실질 no-op. 신규 unit 3 케이스 — (a) 2026-12-24 Christmas Eve 에 fake Christmas firstVespers 적용 확인, (b) solemnity 부재 시 기존 Saturday→Sunday 경로 유지, (c) Solemnity 와 Sunday First Vespers 동시 매치 시 precedence (Solemnity 우선). `npx vitest run` 248 PASS (+3) / `npx tsc --noEmit` 0. **Phase 3b (task #22, 완료, commit 328f14c)**: PDF 27건 solemnity firstVespers 추출 스크립트 + `sanctoral/solemnities.json` 주입 + verifier. movable solemnity 8건 (ascension / pentecost / trinitySunday / corpusChristi / sacredHeart / christTheKing + palmSunday + baptismOfTheLord) 은 extractor `movable` 버킷에 보존 — palmSunday/baptismOfTheLord 는 Phase 2 의 lentW6/christmasBaptism 키로 이미 커버, 잔여 6건은 Phase 4 대상. **Phase 4a (task #23, 완료)**: movable solemnity resolver 확장. `src/lib/propers-loader.ts` 에 `resolveSpecialKey(season, celebrationName)` 헬퍼 신설 (기존 Easter special-key 인라인 로직 추출 + OT 시즌 추가). `getSeasonHourPropers` + `getSeasonFirstVespers` 둘 다 이 헬퍼로 통합 — Easter (easterSunday/ascension/pentecost) 외 OT 시즌 4종 (trinitySunday/corpusChristi/sacredHeart/christTheKing) 매칭 추가. celebrationName 은 permissive substring 기반 (`'trinity'` / `'corpus christi'|'body and blood'` / `'sacred heart'` / `'christ the king'|'king of the universe'`). `src/lib/loth-service.ts` evening-before-solemnity 블록 확장: `getSanctoralPropers(MM-DD)` 가 null (movable 은 sanctoral 에 없음) 일 때 `getSeasonFirstVespers(tomorrowDay.season, tomorrowDay.weekOfSeason, tomorrowStr, tomorrowDay.name)` 로 fallback — 이 경로가 Easter + OT special-key 모두 담당. 우선순위 체계 unchanged (Solemnity First Vespers > Sunday First Vespers > regular fallback). 신규 unit 6 케이스 — (a) 2026-05-13 Wed eve → Ascension Thu, (b) 2026-05-23 Sat eve → Pentecost, (c) 2026-05-30 Sat eve → Trinity Sunday (신규 OT 키), (d) 2026-11-21 Sat eve → Christ the King (신규 OT 키), (e) OT movable name 전부 no-throw lookup flow, (f) 무관 OT name 은 per-week fallback 유지. `npx vitest run` 257 PASS (+6 vs 251) / `npx tsc --noEmit` 0. **Phase 4b (task #24, 완료)**: movable 6건 firstVespers 데이터 주입. Phase 3b extractor 의 `movable` 버킷에서 대상 6건 (ascension / pentecost / trinitySunday / corpusChristi / sacredHeart / christTheKing) 을 routing — Easter movables 는 `propers/easter.json` 기존 `weeks['ascension'\|'pentecost'].SUN` 엔트리에 `firstVespers` 필드 병합, OT 4종은 `propers/ordinary-time.json` 에 신규 special-key `weeks['trinitySunday'\|'corpusChristi'\|'sacredHeart'\|'christTheKing'].SUN.firstVespers` 로 삽입 (신규 4 special key 생성). antiphon_key prefix 는 `movable-<slug>-<ps1\|ps2\|cant>` 로 sanctoral fixed key (`solemnity-*` / `feast-*`) 와 namespace 분리. 스크립트: `scripts/inject-movable-first-vespers.js` (Phase 3b injector 패턴 재사용) + `scripts/verify-movable-first-vespers.js` (PDF 재추출 + byte-equal diff, NFR-009c). 실측: pentecost 만 full 블록 (psalms[3] + shortReading + intercessions + antiphon + 2 prayers), 나머지 5건은 antiphon + concludingPrayer + (대부분) alternativeConcludingPrayer — PDF 의 실제 기재 형태로 ascension/trinity/corpusChristi/sacredHeart/christTheKing 의 1st Vespers 가 공통 seasonal form 에 의존해 짧게 끝나기 때문 (Phase 3b 의 fixed 솔레니티 중 08-06/03-19/08-15/12-08 등도 동일 패턴). `getSeasonFirstVespers` 가 celebrationName → `resolveSpecialKey` → `weeks[specialKey].SUN.firstVespers` 를 자동 반환 (Phase 4a 이미 wired). `loth-service.ts` evening-before-solemnity fallback 이 path 1 (sanctoral MM-DD) null → path 2 (season propers special key) 로 자동 승계. 신규 unit 7 케이스 — (a) ascension / (b) pentecost full psalter / (c) trinitySunday / (d) corpusChristi permissive name matching / (e) sacredHeart / (f) christTheKing short+long name / (g) unrelated OT Sunday 은 weekly fallback 유지. 신규 e2e 5 케이스 (`e2e/movable-first-vespers.spec.ts`) — ascension eve 2026-05-13 antiphon + prayer, pentecost eve 2026-05-23 Ps 113+147+Rev15 psalmody + antiphon, trinity eve 2026-05-30 antiphon + prayer, christTheKing eve 2026-11-21 antiphon + prayer. `npx vitest run` 264 PASS (+7 vs 257) / `npx tsc --noEmit` 0 errors / `verify-movable-first-vespers.js` mismatch=0 PASS / 기존 `verify-solemnity-first-vespers.js` + `verify-first-vespers.js` 회귀 없음. movable 잔여 2건 (palmSunday / baptismOfTheLord) 은 Phase 2 의 lentW6 / christmasBaptism 키로 이미 커버, 추출기 `movable` 버킷에는 남아 있으나 inject 스크립트가 `out-of-scope-for-phase-4b` 사유로 SKIP. **Phase 4c (task #25, 완료)**: Phase 4b 검증 중 포착된 Palm Sunday Eve (2026-03-28) ps1 antiphon 회귀 수정. **증상**: ps1 antiphon 이 기대 `lentPassionSunday` variant ("Сүмд өдөр бүр та нартай хамт байж...") 대신 default_antiphon Easter alleluia variant ("Аяа Эзэн минь, Таны үг бол...Аллэлуяа!") 로 렌더. **원인**: Phase 3a (commit 8d76853) 에서 추가된 evening-before-solemnity 분기가 `seasonPropers` override 할 때 `effectiveDayOfWeek` / `effectiveWeekOfSeason` promotion 을 생략함. Palm Sunday 는 `rank=SOLEMNITY` 라 이 분기가 Saturday→Sunday 분기보다 먼저 fire 하고 후자 (line 200-204 의 `effectiveDayOfWeek='SUN'` + `effectiveWeekOfSeason=nextWeek` promotion) 를 skip. 결과 `pickSeasonalVariant` 가 (SAT, W5) context 로 호출되어 `lentPassionSunday` 게이트 (`dayOfWeek==='SUN' && weekOfSeason in {5,6}`) 미충족 → `default_antiphon` (Easter variant) 으로 폴백. Phase 3a 당시 "solemnity propers ship full antiphons" 가정은 fixed sanctoral solemnity (Christmas 등 seasonal_antiphons 없음) 에서는 맞지만, movable solemnity (Palm Sunday) 가 path 2 `getSeasonFirstVespers(LENT, 6, ...)` → `weeks['6'].SUN.firstVespers` (Phase 2 per-week data with `seasonal_antiphons.lentPassionSunday`) 로 resolve 하는 경로에서는 틀렸음. Phase 4a+4b 이전부터 latent 했던 회귀. **수정**: `loth-service.ts` solemnity 분기에 `effectiveDayOfWeek = dateToDayOfWeek(tomorrowStr)` + `effectiveWeekOfSeason = tomorrowDay.weekOfSeason` promotion 추가 — Saturday→Sunday 분기와 동일 semantics ("liturgical identity of the eve IS tomorrow's day"). Fixed sanctoral solemnity psalms 는 `seasonal_antiphons` 부재로 promotion no-op (Christmas 12-25, Assumption 08-15 회귀 없음). 신규 unit regression guard 1 케이스 (2026-03-28 ps1 antiphon 실측 확인 — real loader, mock 없이 PRD 기재값 byte-equal 매치). **결과**: `npx vitest run` 265 PASS (+1 vs 264) / `npx tsc --noEmit` 0 errors / `e2e/first-vespers.spec.ts` + `e2e/solemnity-first-vespers.spec.ts` + `e2e/movable-first-vespers.spec.ts` + `e2e/easter-antiphon.spec.ts` 16/16 PASS / verifier 3종 mismatch=0 유지. **task #30 (FEAST rank 확장, 완료)**: evening-before 분기의 `tomorrowDay.rank === 'SOLEMNITY'` 조건을 `SOLEMNITY || FEAST` 로 완화해 이미 Phase 3b 에서 주입된 4 feast entries (02-02 Presentation / 08-06 Transfiguration / 09-14 Exaltation of the Holy Cross / 11-09 Lateran Basilica) 의 `firstVespers` 가 전날 저녁에 surface 하도록 활성화. GILH/GIRM 상 FEAST 는 통상 1st Vespers 가 없으나 상기 4건만 PDF 원문이 authored — Path 1 (sanctoral.firstVespers 존재 체크) 이 data-driven activation signal 역할. Path 2 (movable special-key via `getSeasonFirstVespers`) 는 SOLEMNITY 전용으로 명시 gate — movable FEAST 버킷은 정의되지 않았고 GILH 상 해당 사례 없음. 신규 unit 5 케이스 (`first-vespers.test.ts` — 4 feast eve 각각 + Path 2 leak 방지 + firstVespers 부재 FEAST 회귀 가드), 신규 e2e 6 케이스 (`e2e/feast-first-vespers.spec.ts` — 4 feast eve Magnificat antiphon + Presentation 의 concluding prayer + FEAST 랭크이지만 firstVespers 미authored 일 경우 레귤러 Sunday vespers 로 fall-through). 데이터 변경 없음 — 4 feast 의 firstVespers 는 Phase 3b (task #22) 에서 이미 `sanctoral/feasts.json` 에 주입됨. `npx vitest run` 270 PASS (+5 vs 265) / `npx tsc --noEmit` 0 errors / e2e 38 PASS (+6) / verifier 3종 (first-vespers/solemnity/movable) mismatch=0 유지. **task #32 (defensive refactor, 완료)**: Phase 4c (task #25) 가 드러낸 "evening-before 분기와 Saturday→Sunday 분기가 동일한 `effectiveDayOfWeek` / `effectiveWeekOfSeason` promotion 을 독립 보유 → 한쪽 누락 silent regression" 구조적 문제 재발 방지. `src/lib/hours/first-vespers-identity.ts::promoteToFirstVespersIdentity(targetDate, targetDayOfWeek, targetWeekOfSeason)` 헬퍼 추출 후 `src/lib/hours/index.ts` barrel 로 공개. evening-before-solemnity-or-feast 분기 + Saturday→Sunday 분기 양쪽 모두 이 헬퍼 단일 호출로 교체. 주석에 "IDENTICAL semantic — 변경 시 함께 움직일 것" 명시 + 헬퍼 docstring 에 FR-156 Phase 4c 회귀 맥락 포함. 순수 refactor — behavior change 0 (`npx vitest run` 273 PASS (+3 helper unit) / `npx tsc --noEmit` 0 / e2e FR-156 matrix 38 PASS 유지 / verifier 3종 mismatch=0 유지). 신규 unit 3 케이스 (`first-vespers-identity.test.ts`). **task #36 (firstVespers page 필드 주입 + verifier 확장, NFR-009d 적용, 완료)**: 71건 firstVespers entries (52 Sunday + 13 solemnity/feast + 6 movable) 에 PDF page 필드 일괄 주입. 신규 `scripts/inject-first-vespers-pages.js` — `propers/*.json` 과 `sanctoral/{solemnities,feasts}.json` 의 firstVespers 서브트리만 walking, 기존 `extract-propers-pages.js` 와 달리 regular (non-firstVespers) 엔트리는 건드리지 않음 (add-only-safe). 페이지 매칭 로직은 기존 `scripts/lib/page-fingerprint.js` + 신규 `scripts/lib/first-vespers-page-annotator.js` (공통 annotator 로 injector 와 verifier 가 공유). annotation 대상 7 필드 카테고리: `concludingPrayerPage` / `alternativeConcludingPrayerPage` / `gospelCanticleAntiphonPage` / `intercessionsPage` / `shortReading.page` / `responsory.page` / `psalms[i].page`. 실측 주입: **332 page fields** (psalms 150 + shortReading 54 + intercessions 54 + responsory 29 + concludingPrayer 19 + gospelCanticleAntiphon 19 + altConcludingPrayer 7). 71/71 블록 각 ≥1 page 필드 보유. verifier 3종 (`verify-{first,solemnity,movable}-first-vespers.js`) 을 확장 — `buildExpectedFirstVespers` 직후 `annotatePagesInPlace(expected, pageIdx)` 호출해 expected 에도 동일 annotation 적용 → 기존 byte-equal diff 가 page 필드 drift 까지 자동 포착 (mismatch=0 유지). 런타임 코드 무수정 — `npx vitest run` 273 PASS (변화 없음) / `npx tsc --noEmit` 0 / FR-156 e2e 38 PASS 유지 / `verify-psalter-pages.js` agree=143 유지. 데이터 변경 outside firstVespers subtree 없음. **task #41 (34 unmatched page 복구 + 27 wrong 값 교정, 완료)**: task #36 의 fingerprint-only 방식이 놓쳤던 34건 unmatched (23 responsory + 11 Ps 113 antiphon) 를 복구하면서 감사 과정에서 발견된 27건 wrong 값 (psalter-1 shortReading 15건 571→55 / psalter-3 responsory 12건 541→292) 동시 교정. **근본 원인**: "Нар мандахаас жаргах хүртэл..." (Ps 113 antiphon) 과 "Аяа, Эзэн, бид Таны үйлсийг..." (psalter-3 responsory) 같은 텍스트가 PDF 여러 섹션에 중복 등장 → fingerprint 매칭이 psalter 밖 occurrence 로 false positive. **해결**: `scripts/lib/first-vespers-page-annotator.js` 에 `PSALTER_WEEK_PAGES` static 맵 추가 (full_pdf.txt 에서 "Шад дуулал"/"Уншлага"/"Хариу залбирал" 마커로 각 4주 psalter 블록의 실제 page 를 직접 읽어 도출 — psalter-1: 49/51/53/55/55, psalter-2: 166/168/170/171/172, psalter-3: 287/289/290/292/292, psalter-4: 398/400/401/402/403). `PSALTER_WEEK_MAPPING` + `getPsalterWeekFromMapping(season, weekKey)` helper export — 52 Sunday propers 엔트리는 대응 psalter week 의 static page 직접 사용. injector (`walkPropers` / `walkSanctoral` 분리) 와 `verify-first-vespers.js` 양쪽 모두 동일한 psalterWeek 컨텍스트 적용. 사원사 (9 solemnity + 4 feast) / movable (6) 엔트리는 fingerprint 유지 — 이들은 PDF section-고유 페이지라 false positive 없음. **결과**: 332 → **369 page fields** (added 37 + changed 27 + unchanged 305). 71/71 blocks 전 필드 커버 (missing 0, wrong 0). `npx tsc --noEmit` 0 / `npx vitest run` 273 PASS / `verify-{first,solemnity,movable}-first-vespers.js` mismatch=0 PASS / `verify-psalter-pages.js` agree=147 유지 / FR-156 e2e 38 PASS 유지. **Phase 5 (bare-ref → versed 데이터 정합화, 완료)**: PDF 본문이 bare 형 ref (`Psalm 122`) 로 기재된 firstVespers cells 가 다음 두 경로 모두 miss 해 빈 시편 카드를 노출하던 회귀 정합 — (1) `psalter-texts.json` catalog 키는 versed 형 (`Psalm 122:1-9`) 만 보유 → bare key 매칭 실패, (2) `scripture-ref-parser.ts` regex 가 콜론 강제 → `parseScriptureRef("Psalm 122")` 빈 배열 반환 → Bible JSONL fallback 도 발화 못함. 결과 `loth-service` 가 placeholder fallback 으로 진입해 SAT vespers 화면에 빈 시편 카드 + (일부 케이스) Magnificat 미렌더. 본 plan: `docs/fr-156-phase5-firstvespers-bare-ref-plan.md`. **결정**: SSOT (`psalter-texts.json` versed 형 invariant) 보존 위해 parser regex 무수정, 데이터 측 normalization 으로 대응. **WI-A1/A2 (task #75/#86/#87)**: `parsed_data/first-vespers-versed-map.json` (7 entries: Psalm 16/113/122/130/142/147 + 141:1-9 sanity) — PDF anchor + body fingerprint dual verification. `scripts/extract-first-vespers-versed-map.js` 추출기 + `scripts/rewrite-first-vespers-bare-refs.js` 자동 rewrite 도구 (18-test 단위 스위트, `--dry-run` / `--season` / `--allow-missing-catalog` 플래그). **WI-A3 (task #74)**: `scripts/verify-first-vespers-ref-coverage.js` 신규 verifier — propers + sanctoral 의 모든 firstVespers `psalms[*].ref` 가 (a) versed-form regex 일치 (b) catalog 키 hit (c) antiphon-slot 일관성 3가지 invariant 검증, 위반 cell 위치 리스트 출력. basal red 81 violations (66 bare + 15 catalog miss). **WI-C (task #88, commit 1356c73)**: catalog 보강 — `psalter-texts.json` 에 Psalm 141:1-9 + Psalm 142:1-7 신규 entry 2건 추가 (PDF stanza 직접 인용, NFR-009b "추측 금지" 준수). **WI-A4 (WI-B1 흡수, task #89)**: byte-equal verifier 3종 (`verify-{first,solemnity,movable}-first-vespers.js`) 의 `diff()` 함수에 `normalizeRef` on `.ref` paths 패치 추가 — `versed-map.json` 의 `rewrite_needed=true` entries 한정으로 PDF extractor canonical bare-form ↔ propers JSON versed-form 동등 처리 (NFR-009c). 한 번 적용 → 모든 후속 WI-B 시즌 자동 수용. **WI-B1~B5 (task #89/#90/#91/#94/#95)**: 5 시즌 데이터 rewrite — easter 8 cells (commit 08ca495), lent 7 cells (a44b9e4), advent 5 cells (cbf351c), christmas + sanctoral 12-25 4 cells (f2fb22b), ordinary-time 42 cells (fd6db42). 시즌별 e2e + integration vitest 추가. **결과**: ref-coverage 81 violations → **0 violations PASS**, byte-equal verifier 3종 mismatch=0 유지, vitest 273 → **332+ PASS**, tsc 0 errors, sw.js 무수정. parser regex 무수정 — verifier-gated normalization 으로 SSOT invariant 영구 enforcement (`docs/fr-156-phase5-firstvespers-bare-ref-plan.md` §7.3 ADR-P5-1/2/3/4 참조). NFR-009b/d 준수 (PDF 본문 직접 인용, byte-equal verifier 가드). **task #96 (WI-D)**: 본 row + traceability matrix + plan §4·§7 갱신 (이 paragraph). **GOAL #20 (#20-sub-2, 완료) — 이동 대축일 2저녁기도(vespers2) 라우팅 (option B)**: 이동 대축일 당일의 2저녁기도가 `/pray/<date>/vespers` 로 렌더되도록 라우팅 추가 — `conditional-rubric-easter.spec.ts` 가 'future PR' 로 미뤘던 vespers2 cell 도달 경로. `DayPropers.vespers2?: HourPropers` 타입 신설(기존 `pentecost.SUN.vespers2` 가 타입상 조회 불가였던 갭 해소) + `getSeasonVespers2(season, week, dateStr, name)` loader + `loth-service.ts` 초기 seasonPropers fetch 직후 swap 분기(`hour==='vespers' && rank==='SOLEMNITY' && resolveSpecialKey != null` → vespers2 블록으로 교체). 고정일 솔렘니티의 기존 `sanctoral.vespers2` swap(L446)의 이동 대축일(season-propers) 대응. 결과: Pentecost `/vespers` 가 EP-I 중복셀(Магнификат "Ариун Сүнс бууж"+독서 Rom 8:9-11+진행주간 week-4 Ps112) 대신 정답 EP-II(Магнификат "Өнөөдөр бид"+독서 Eph 4:3-6+week-1 SUN Ps110/**Ps114**/Rev19) 렌더. Trinity/Corpus/Sacred Heart/Christ the King/Ascension 5 대축일 2저녁기도 일괄 활성(평일 Sacred Heart 금/Ascension 목 포함, SUN-슬롯 fallback). `@fr FR-156`. | 데이터 / 파이프라인 / 아키텍처 | P2 | Phase 1/2/3a/3b/4a/4b/4c + task #30 + task #32 refactor + task #36 page 주입 + task #41 page gap 복구 + Phase 5 bare-ref 정합화 (66 cells 5 시즌) + GOAL #20 vespers2 routing(option B) 완료 |

| FR-162 | **Footer 출처표시 항상 노출 (토글 제거 — #51/#53 갱신)**: 〔현행 (GOAL #54, #63)〕 footer 의 click-to-toggle(▾/▴) chevron 을 **제거**하고 교회 출처표시 2줄('Цагийн Залбирал — Монгол Католик Сүм' / 'Зарим орчуулга хийгдэж байна')을 항상 작은 caption(text-stone-400) 으로 노출한다 — 토글 뒤에 숨어 발견성이 낮던 문제 해소. 홈 변형은 컨트롤 2개(Өнөөдөр / Тохиргоо, lucide 아이콘+라벨)만 유지하고 토글 chevron 은 삭제. `data-role="footer-toggle"` / `aria-expanded` / `aria-controls` / expanded `useState` 전부 제거 (`data-role="footer-content"` 출처 컨테이너는 상시 렌더로 유지). 아이콘은 lucide 단일 패밀리(`<Icon>`) — 이모지(⊙⚙)/유니코드 글리프 0 (DESIGN.md Iconography). 단위 `src/components/__tests__/footer.test.ts` (토글 부재 + 출처 상시 노출 + 2 컨트롤 + WCAG focus-ring 검증), e2e `e2e/footer-toggle.spec.ts` (`@fr FR-162` — 토글/aria-expanded 부재 + 출처 2줄 상시 노출). 〔이력 (task #11)〕 최초엔 클릭 토글(보였다가 숨김)로 도입 — 사용자 directive (2026-05-14 — "하단의 풋터는 클리하면 보였다가 클릭하면 숨기는 식으로 작동하게 하면 좋겠어") 응답. `src/components/footer.tsx` 를 client component (`'use client'`) 로 전환하고 `useState` 로 자체 expanded 상태 관리 — 기본 상태 collapsed (작은 chevron `▾` toggle 버튼만 surface, 2 줄 credit 본문은 DOM 에 부재), 1번째 클릭 → expanded (chevron `▴` + 본문 두 줄 surface), 2번째 클릭 → collapsed 로 복귀. 옵션 B 채택 (작은 handle 만 default 로 표시) — 옵션 A 가 "풋터 전체 영역 클릭 가능" 이면 collapsed 시 클릭 대상이 사라져 사용자가 토글을 다시 못 부르는 dead-end 발생. 시그니처 `<Footer />` (props 없음) 유지 — 호출부 5곳 (`src/app/page.tsx`, `settings/page.tsx`, `guide/page.tsx`, `ordinarium/page.tsx`, `pray/[date]/[hour]/page.tsx`) 무수정. **접근성**: 네이티브 `<button type="button">` 으로 Tab focus + Space/Enter 토글 무료, `aria-expanded` reflects toggle state, `aria-controls` 이 expanded 시 surface 되는 `data-role="footer-content"` 컨테이너 id 를 가리킨다 (collapsed 상태에서도 id 사전 약속 유지). aria-label 은 Mongolian Cyrillic 한정 — "Доод бичвэр харуулах" / "Доод бичвэр нуух" 으로 상태별 swap, 영어 fallback 0건 (NFR-002). 단위 (`src/components/__tests__/footer.test.ts`, 7 case, react-dom/server SSR pattern — 기존 marian-antiphon-section.test.ts 와 동일 — collapsed default 의 ARIA / data-role / Mongolian 문구 / focus 보존 invariant 만 assertion, 클릭 transition 은 e2e). e2e (`e2e/footer-toggle.spec.ts`, 5 case, `@fr FR-162`) — default 상태 / 1차 클릭 expand / 2차 클릭 collapse / 키보드 Tab→Space→Enter 토글 / aria-label 몽골어 swap. light + dark 양쪽 viewport 1024×768 캡처 4종 (`/tmp/dvo-ref-footer-{light,dark}-{collapsed,expanded}.png`) PASS — chevron 색·hover bg 가 stone-400/200 (light) ↔ stone-500/700 (dark) 으로 정확 swap. **SW 캐시 영향 0**: 링크 스키마 변경 ❌ / 새 라우트 ❌ / Content-Type 변경 ❌ / public/ 자산 경로 변경 ❌ — Navigation 은 SW network-only (`public/sw.js` line 357-365, CLAUDE.md 정책) 라 HTML 변경은 캐시되지 않고 상시 fresh 서빙. `CACHE_VERSION` bump 불필요 (divine-office-v29 유지). 〔#63 갱신〕 토글 제거·출처 상시 노출은 route/asset/Content-Type 무변경 → CACHE_VERSION bump 불요(navigation network-only 유지). | UI | P3 | 완료 (task #11) → #51/#53 토글 제거·출처 상시 노출 갱신 (#63, member: dvo-ref) |

| FR-163 | **첫 화면 전례력 월간 표시 + 달 네비게이션 (GOAL #4)**: 사용자 directive (2026-05-19, GOAL #4 plan_artifact `msg_20260519T1350250000_4257`) — 첫 화면 (`/`) 의 'today-centric infinite scroll' (FR-145 iter 1) 을 'month-mode (한 달씩 끊어 보기 + 달 네비게이션)' 로 전환. 5 가지 통합 요구: **D1** ?month=YYYY-MM URL 로 한 달치만 표시 — 페이지가 1일 ~ 말일 범위 rows 만 server-side 로 결정해 클라이언트에 전달. **D2** 첫 방문 / `?date=` 만 있을 때 today 가 표시 영역에 자동 anchor — `LiturgicalCalendarList` 의 mount-once `scrollIntoView({block:'center'})` 가 focusDate (`initialDate ?? todayStr`) 의 row 를 화면 중앙으로 스크롤. focusDate 가 현재 month 의 rows 에 없으면 ref 미할당 → no-op (사용자가 미래 월로 네비게이트한 자연스러운 상태). **D3** 상단 헤더에 prev/next/picker 컨트롤 `MonthNav` — 라벨 'YYYY оны M-р сар' 몽골어 (`formatMonthLabel` 헬퍼) + ◀ Өмнөх сар + Дараах сар ▶ + native `<input type="month">` 피커 (라벨 탭 → `showPicker()` + iOS Safari focus fallback). prev/next 는 `shiftMonth(±1)` (연도 경계 1월 ↔ 12월 + 멀티-year 점프 동일 산식, JS 음수 modulo `(t%12+12)%12+1`). URL push 는 컴포넌트 분리 원칙으로 호출자 (page.tsx wrapper) 담당 — unit test 가 URL 의존성 없이 컴포넌트 동작만 가드. **D4** 무한 스크롤 메커니즘 전부 제거 — `LiturgicalCalendarList` 의 `IntersectionObserver` + `loadOlder`/`loadNewer` + `hasScrolled` + `exhausted*`/`loading*` state + top/bottom sentinel + `window.addEventListener('scroll', ...)` 모두 삭제 (production net -155 LOC). **D5** 상단 헤더의 SettingsLink 제거 — 본 라우트 레벨에서 settings 진입은 footer (FR-162) 또는 별도 surface 로 대체 (구현 디테일은 wi-004 결정). **컴포넌트 매핑**: `src/components/month-nav.tsx` (신규 162 LOC) — 라벨/prev/next/picker + 5개 aria-label 몽골어 + aria-haspopup='dialog' + min 44×44 탭 타깃 + focus-visible:outline-2 outline-offset-2 outline-stone-400 + data-testid (month-nav, month-nav-prev, month-nav-label, month-nav-label-text, month-nav-next, month-nav-picker) + data-role='month-nav' (CLAUDE.md selector 원칙). 헬퍼 `shiftMonth(ym, delta)` + `formatMonthLabel(ym)` 는 export 되어 단위 테스트 직접 호출 가드. `src/components/liturgical-calendar-list.tsx` (refactor 41/196) — month-bounded `initialRows` 그대로 렌더 + mount-once autoscroll + `useEffect deps=[]` (이전 `[rows]` 는 페이징 시 재스크롤 불필요 회귀) + `${row.kind}-${row.date}` key (anchor vs today fiber 분리) + handleToggle kind 가드. `src/app/page.tsx` — `?month=YYYY-MM` 라우팅 + `getCalendarMonth(yyyyMM, todayStr)` 어댑터 호출 + ±60일 윈도우 제거 + MonthNav header wiring (router.push wrapper). `src/lib/calendar-list.ts::getCalendarMonth` — 1일 ~ 말일 범위 rows 빌더 (#13 wi-001 dvo-dev-cl). **단위 (`src/components/__tests__/month-nav.test.ts`, 15 case + `liturgical-calendar-list.test.ts`, 7 case)** — react-dom/server SSR 패턴 (gospel-canticle-section / liturgical-calendar-list 컨벤션). 클릭 시뮬레이션은 jsdom 미설치라 헬퍼 단위 테스트로 대체 (`shiftMonth` boundary 6 case + `formatMonthLabel` 8 case + 정적 마크업 a11y / focus-visible / chevron / picker controlled value). e2e (`wi-008` #20 — Playwright mobile + desktop) 가 dynamic showPicker 동작 + 실기기 ?month= 라우팅 + autoscroll 시각 가드 예정. **SW 캐시 영향**: navigation (HTML) 은 `public/sw.js` `network-only` (line 473-481 in v39) 라 ?month= URL 신규 cache 처리 불요 — `request.mode === 'navigate'` 분기가 별도 path 매칭 없이 자동 처리, `sw.test.ts` 회귀 가드 1 case 추가. 정적 자산 (script chunks: `page` / `liturgical-calendar-list` / `month-nav` 신규) hash 변동 → `CACHE_VERSION` v38 → v39 conservative bump (v38 잔존 시 구 LiturgicalCalendarList chunk 가 cache-first 로 무한 서빙되어 month-mode 페이지 정상 동작 못 함). FR-163 ↔ FR-145 supersede relation — FR-145 (today-centric infinite scroll) 는 'iter 1 결정' 으로 보존, FR-163 가 'iter 2 결정 (GOAL #4 의 D1-D5 통합 요구)' 으로 갱신. **WI 분해**: wi-001 #13 (getCalendarMonth 어댑터, dvo-dev-cl 완료) / wi-002 #14 (page.tsx 라우팅, dvo-dev-cl 완료) / wi-003 #15 (MonthNav 컴포넌트, dvo-ref 완료) / wi-004 #16 (page.tsx header 통합, dvo-dev-cl 진행) / wi-005 #17 (LiturgicalCalendarList infinite scroll 제거, dvo-ref 완료) / wi-006 (Footer sticky — 별도 GOAL #24 로 흡수) / **wi-007 #19 (SW + 문서 정리, dvo-ref 진행)** / wi-008 #20 (Playwright e2e pending) / wi-009 #21 (통합 검증 pending). | UI / 아키텍처 | P2 | wi-001/002/003/005 완료, wi-004/007 진행, wi-008/009 pending |

| FR-164 | **기도문 화면 PrayerFooter — 하단 strip 탭 → 전례력/설정 메뉴 슬라이드 (GOAL #24)**: 사용자 directive (2026-05-19, GOAL #24 plan_artifact `msg_20260519T1540290000_dbef`) — 기도문 화면 (`/pray/[date]/[hour]`) 상단의 ⚙ SettingsLink 가 진입점을 분산시키고 작은 탭 영역으로 어르신 접근성 불리. 하단에 32px sticky strip + 탭 시 slide-up 패널 (Огноо + Тохиргоо) 도입으로 진입점 단일화 + 큰 탭 영역 확보. 5 가지 결정: **D1=B** 32px bottom strip (sticky) 탭 → slide-up panel (옵션 B 채택, 화면 상시 점유 vs 슬라이드 reveal trade-off 의 후자). **D2=a** 메뉴 항목 최소 ({📅 Огноо, ⚙ Тохиргоо} 2개) — 가독성 + 어르신 단순 UI. **D3=icon+text** 메뉴 항목은 emoji 아이콘 + 몽골어 라벨 (📅 Огноо / ⚙ Тохиргоо). **D4=b** 별개 PrayerFooter 컴포넌트 — 기존 `<Footer />` (FR-162 home credit chevron, normal flow) 와 분리되어 prayer page 만 PrayerFooter (fixed bottom) 마운트. 두 footer 가 z-index / position 으로 시각 충돌 없이 공존. **D5=a** 상단 SettingsLink 제거 — 진입점 PrayerFooter 의 [⚙ Тохиргоо] 메뉴로 단일화. **컴포넌트**: `src/components/prayer-footer.tsx` (신규 +163 LOC) — controlled/uncontrolled hybrid (React input pattern, expandedProp 미지정 시 내부 useState) + always-mounted panel + translate-y class swap (collapsed: translate-y-full + aria-hidden=true + inert / expanded: translate-y-0 + aria-hidden=false) + slide-up animation (`transition-transform duration-200 ease-out` + `motion-reduce:transition-none motion-reduce:duration-0`) + backdrop element (outside-tap + 시각 dim, bg-black/30 + opacity transition) + Esc key dismiss (useEffect window keydown listener with cleanup, Escape → handleClose + stripRef.focus() 복귀) + focus management (firstMenuRef.focus() on expand) + 5 aria-label 몽골어 default (Өмнөх / Дараах / Сар сонгох 등 적절 라벨) + aria-haspopup='dialog' + min 44×44 탭 타깃 + focus-visible:outline + data-role anchors (prayer-footer / -strip / -content / -menu-date / -menu-settings / -backdrop). **page.tsx 통합** (`src/app/pray/[date]/[hour]/page.tsx` +32 / -8): import SettingsLink 제거 + import PrayerFooter 추가, 상단 toolbar `justify-between` → `items-center` (Back link 만 유지), 외곽 div 에 `pb-16` (strip 가림 방지), <Footer /> (FR-162 보존) 직후 <PrayerFooter date={date} celebration={footerCelebration} /> 마운트. footerCelebration = `celebration && celebration !== 'default' ? celebration : undefined` (Buцах link convention 정합 + PrayerFooter 내부 truthy check 이중 방어). **loading.tsx 갱신** (+45 / -28): Fragment wrapping + pb-16 + settings-icon skeleton 제거 + 신규 PrayerFooter strip skeleton (`data-role="prayer-footer-strip-skeleton"` fixed bottom-0 z-40 h-[32px], 실제 컴포넌트와 동일 geometry — layout shift 회피). **단위 (`src/components/__tests__/prayer-footer.test.ts`, 23 case)** — react-dom/server SSR 패턴 + 인터랙션 패턴 가드 (옵션 B leader directive, msg_20260520T... ack): WI-A 16 (always-mounted 전환에 따라 panel-DOM 부재 단언 2개를 panel-mounted + translate-y-full 단언으로 정정) + WI-B 7 신규 (uncontrolled/controlled mode 분기 / always-mounted invariant / motion-reduce CSS / backdrop element / source-grep × 2 = useState + useEffect + 'keydown' + 'Escape' + stripRef.focus() + firstMenuRef.focus() + handleClose + inert + isControlled 모두 존재). 실제 라이브 인터랙션 (toggle / Esc / outside-tap / focus / reduced-motion) 은 WI-E #33 Playwright e2e (mobile + desktop + reduced-motion viewport) 책임. **SW 캐시 영향**: navigation (HTML) 은 `public/sw.js` `network-only` (line 495-503 in v40 — v39 의 line 473 reference 가 본 bump 코멘트 +22 line shift, FR-163 정의 유지) — 라우트 / Content-Type 변경 없음. 그러나 신규 client 컴포넌트 (prayer-footer chunk) + page.tsx / loading.tsx 수정 → script chunks hash 변동 → `CACHE_VERSION` v39 → v40 conservative bump (v19 GOAL #4 bump 와 동일 패턴, v39 잔존 시 구 prayer page chunk 가 cache-first 무한 서빙되어 상단 SettingsLink 잔존 + footer strip 부재 깨진 UX 노출). PRECACHE_URLS = ['/offline.html', '/icon.svg'] 변동 없음. FR-164 ↔ FR-162 supersede relation 없음 — FR-162 (home Footer credit chevron) 는 PrayerFooter 와 별개 컴포넌트로 prayer page 에 공존 (D4=b). **WI 분해**: wi-A #29 (PrayerFooter 컴포넌트 신규, dvo-ref 완료) / wi-B #30 (인터랙션 로직 controlled/uncontrolled hybrid + slide animation + Esc/outside-tap dismiss + focus management + motion-reduce, dvo-ref 완료) / wi-C #31 (page.tsx 통합 + 상단 SettingsLink 제거 + loading.tsx skeleton, dvo-ref 완료) / **wi-D #32 (본 WI — SW + 문서 정리 + FR 부여, dvo-ref 진행)** / wi-E #33 (Playwright e2e mobile + desktop + reduced-motion + WI-30 reviewer 가이드 'backdrop-close 후 document.activeElement' 단언 권장, pending). | UI / 접근성 | P2 | wi-A/B/C 완료, wi-D 진행, wi-E pending |

| FR-165 | **글자 크기 단계 확장 (XS→5XL) + settings stepper UI — 어르신 가독성 (GOAL #34 → #50 확장)**: 〔현행 상태 (GOAL #50 — WI #52 merged `26fec4f` + docs WI #56)〕 fontSize 는 **9단계**: `xs`/`sm`/`md`/`lg`/`xl`/`xxl`/`xxxl`/`x4l`/`x5l` (stepper label XS/S/M/L/XL/XXL/XXXL/4XL/5XL = 87.5/93.75/100/112.5/125/137.5/**150/175/200**%). **천장 = 5XL 200%** — `[Aa+]` 는 5XL(x5l) 에서, `[Aa−]` 는 XS(xs) 에서 disabled, **9단계 순회**. 구현 SSOT: `src/lib/settings.tsx` FontSize union(9) + FONT_SIZES, `src/app/globals.css` `html[data-font-size="xxxl|x4l|x5l"]` = 150/175/200%, `src/app/settings/page.tsx` stepper FONT_SIZES(9). 아래 〔GOAL #34 이력〕은 최초 6단계(천장 XXL=137.5%) 도입 기록이며, GOAL #50 이 xxxl/x4l/x5l 3단계를 더해 천장을 200% 로 확장함. 〔GOAL #34 이력〕 사용자 directive (2026-05-19 — "그리고 글자 크기가 더 키울 수 있으면 좋겠어 어르신들은 더 크게 보던데.") + 2026-05-20 follow-up (AskUser 응답 — "알고 있으나 XL(125%) 도 부족" → XXL 단계 추가 확정 / UI 위치 = Settings 유지 / Stepper 형태 = 옵션② stepper 만). 기존 5단계 (`xs`/`sm`/`md`/`lg`/`xl` = 87.5/93.75/100/112.5/125%, `src/app/globals.css` L45-49 + `src/lib/settings.tsx` FontSize union, 이미 구현) 위에 **XXL=137.5%** 신규 단계를 얹어 어르신 (PDF "아저씨/어르신" 사용자 군) 가독성 천장을 높이고, settings 페이지의 5-radio UI 를 3-button stepper ([Aa−] [Aa] [Aa+]) 로 재구성해 단순화. 5 가지 결정 (F1-F5): **F1=settings 유지** — UI 진입점은 기존 `/settings` 페이지의 `Үсгийн хэмжээ` 섹션 (footer 통합/floating button 옵션 둘 다 reject — 어르신 단순 UI 원칙 + 기존 settings 패턴 일관). **F2=6단계 (GOAL #34 당시 — 현재 9단계, 위 현행 상태 참조)** — `xs`/`sm`/`md`/`lg`/`xl`/`xxl` (87.5/93.75/100/112.5/125/137.5%). 기존 step 분포는 균일하지 않음 — xs→sm 6.25% / sm→md 6.25% / md→lg 12.5% / lg→xl 12.5% (하위 두 단계 6.25%, 상위 두 단계 12.5%). XXL 137.5% 는 XL+12.5% — 상위 (md~xl) 의 12.5% step 컨벤션을 연장. PDF 본문 검증 (어르신 모드 가독 한계) 기반. **F2-form=stepper** — `[Aa−] [Aa] [Aa+]` 3-button stepper + "Одоо: <단계>" indicator + 6단계 순회 (xs↔xxl 양 끝 경계에선 해당 방향 button disabled — GOAL #34 당시; **현재 9단계 순회 xs↔x5l**). 옵션① 5-radio 유지는 reject (단계 증가로 UI 가로 압박) / 옵션② stepper 채택. **F3=전체 앱** — `<html data-font-size="xxl">` root-level data-attribute 가 globals.css 의 `html[data-font-size="..."] { font-size: ... }` 를 통해 root font-size 를 조정하므로 모든 컴포넌트 (시편 본문, 안티폰, 복음경문, 응송, 기도, 메뉴, 라벨) 자동 적용 — 이미 구현된 메커니즘에 xxl rule 1줄 추가만으로 확장. **F4=localStorage** — `'loth-settings'` 키의 `fontSize` 필드 (이미 `src/lib/settings.tsx` 의 `SettingsState` schema + `loadSettings` / `saveSettings` 헬퍼). xxl 신규 union member 추가 + migration (legacy non-xxl 값은 그대로 보존, xxl 은 신규 저장 시점부터 발화) — schema 검증은 `FONT_SIZES` const 의 union 그대로 사용. **F5=data-attribute + CSS variable** — 이미 구현 (`<html data-font-size>` + `globals.css` `html[data-font-size="<size>"] { font-size: <%>; }` rule). xxl rule 1줄 추가 (`html[data-font-size="xxl"] { font-size: 137.5%; }`) 만으로 신규 단계 발화. DOGFOODING AC (사용자 명시 1:1 매핑): **D1** 어르신 사용자가 글자를 더 크게 볼 수 있다 → XXL 137.5% 옵션 신규 (XL 125% 보다 큰 단계 존재) — settings 페이지 stepper 의 우측 끝에서 도달. **D2** 사용자가 직접 글자 크기를 조절할 수 있다 → settings page stepper `[Aa−] [Aa] [Aa+]` 로 조절. `[Aa−]` 탭 → 한 단계 감소 (xs 도달 시 disabled), `[Aa+]` 탭 → 한 단계 증가 (xxl 도달 시 disabled — GOAL #34 당시; **현재 천장 x5l 200% 도달 시 disabled**), 중앙 `[Aa]` 는 현재 단계 indicator. **D3** 기도문 본문 (시편 / 안티폰 / 복음경문 등) 에 적용 → `<html data-font-size>` 가 root font-size 이므로 자동 적용 (`rem` 단위 기반 컴포넌트 일괄 scale). **D4** 설정이 reload 후에도 유지 → `localStorage 'loth-settings'` (기존 메커니즘, xxl union 추가만으로 자동 persist). **D5** 모바일 / 데스크탑 양쪽 적용 → 글로벌 CSS scale 이므로 viewport 무관 적용. **컴포넌트 매핑**: `src/lib/settings.tsx` (FontSize union `'xs'|'sm'|'md'|'lg'|'xl'` → `+ 'xxl'`, FONT_SIZES const + loadSettings/saveSettings/getEffectiveFontSize 호환) — WI-A 책임. `src/app/globals.css` (L44-49 fontSize block 에 `html[data-font-size="xxl"] { font-size: 137.5%; }` 1줄 추가) — WI-A 책임. `src/app/settings/page.tsx` (Үсгийн хэмжээ 섹션 — 5-radio fieldset → 3-button stepper + "Одоо: <단계>" indicator + decrement/increment 핸들러 + 경계 disabled) — WI-B 책임. **단위 (`src/lib/__tests__/settings.test.ts`)** — FontSize union 확장 가드 (xxl 포함) + loadSettings/saveSettings xxl roundtrip + legacy 5-step 값 (xs/sm/md/lg/xl) 로딩 시 그대로 보존 회귀 가드 — WI-A 책임. **e2e (`e2e/settings.spec.ts`)** — stepper 3-button 노출 + xs↔xxl 양 끝 disabled / 6단계 순회 / xxl 선택 시 `html[data-font-size="xxl"]` reflect / reload 후 xxl 유지 — WI-B 책임. **SW 캐시 영향**: navigation (HTML) 은 `public/sw.js` `network-only` (line 495-503 in v40, FR-163/FR-164 정의 유지) — 라우트 / Content-Type / 자산 경로 / 외부 링크 / `<a href>` 스키마 모두 변경 0. 본 GOAL #34 변경 분석: (1) `src/lib/settings.tsx` TypeScript 타입 union 확장 — Next.js 빌드가 해당 chunk hash 를 자동 재발급. (2) `src/app/globals.css` xxl rule 1줄 추가 — Next.js 빌드가 `globals-[hash].css` filename hash 자동 변동 (cache.match 자연 미스 → 새 fetch + 새 캐시). (3) `src/app/settings/page.tsx` UI 재구성 (5-radio → stepper) — settings page chunk hash 자동 재발급. **CACHE_VERSION bump 결정 = 불요**. 근거 — (a) FR-163 (v38→v39) / FR-164 (v39→v40) 의 bump trigger 는 "신규 client 컴포넌트" (`month-nav.tsx` / `prayer-footer.tsx`) 도입 + page 통합으로 인한 _구_ chunk 의 cache-first 무한 서빙 risk 였음. 본 GOAL #34 는 신규 컴포넌트 도입 없음 — settings.tsx + globals.css + settings/page.tsx 셋 다 _기존_ 파일 수정. (b) Next.js 빌드의 hashed asset 컨벤션 (chunks-[hash].js / globals-[hash].css) 이 file name 단위로 invalidation 을 자연 처리 — SW 의 fetch handler (line 506-528) 가 destination ∈ {script,style,font,image} 에 cache-first 적용하더라도 새 hash filename 은 cache.match miss → fetch → cache.put 으로 자동 갱신. (c) navigation HTML 은 항상 network-only (line 495-503) 라 SSR 출력 (xxl 옵션이 포함된 stepper DOM) 이 즉시 갱신. (d) PRECACHE_URLS = ['/offline.html', '/icon.svg'] 변동 없음. (e) `divine-office-v40` 잔존 시나리오에서도 globals-[oldhash].css / chunks-[oldhash].js 가 새 layout.tsx / page.tsx HTML 의 새 asset URL 과 자연 mismatch → 새 hash 자산을 fetch — 사용자가 "구 5-radio + XL 까지" UI 에 갇히는 회귀 risk 0. 따라서 v40 유지, sw.js + sw.test.ts 무수정. FR-165 ↔ FR-164/FR-163/FR-162 supersede relation 없음 — 직교 기능군 (글자 크기 = settings 가독성, FR-164 = prayer page footer, FR-163 = home calendar, FR-162 = home credit). **WI 분해**: **wi-A #45 (dvo-ref, 진행)** — `src/lib/settings.tsx` FontSize union 확장 `+ 'xxl'` + FONT_SIZES const 갱신 + `src/app/globals.css` `html[data-font-size="xxl"] { font-size: 137.5%; }` 1줄 추가 + `src/lib/__tests__/settings.test.ts` xxl union 가드 + legacy 5-step migration 회귀 가드. **wi-B #46 (BLOCKED-BY #45)** — `src/app/settings/page.tsx` Үсгийн хэмжээ 섹션 5-radio → 3-button stepper UI 재구성 (decrement/increment 핸들러 + 경계 disabled + "Одоо: <단계>" indicator 몽골어 라벨 + min 44×44 탭 타깃 + focus-visible:outline + data-role anchors) + `e2e/settings.spec.ts` stepper + 6단계 순회 + xxl reflect + reload 유지 케이스. **wi-C #47 (dvo-dev-cl, 진행 — 본 WI)** — PRD FR-165 + traceability matrix FR-165 행 신규 추가 + sw.js navigation 정책 + 본 GOAL 의 SW 영향 reasoning 검토 + CACHE_VERSION bump 불요 결정 기록. WI-A 와 병렬 진행 (의존 없음 — 결정 사항 명세는 본 WI 가 독립적으로 작성). **〔GOAL #50 확장 WI 분해〕**: **#52 (dvo-dev-cl, merged `26fec4f`)** — `src/lib/settings.tsx` FontSize union + FONT_SIZES 에 `xxxl`/`x4l`/`x5l` 추가, `src/app/globals.css` `html[data-font-size="xxxl|x4l|x5l"]` = 150/175/200% 3 rule, `src/app/settings/page.tsx` stepper FONT_SIZES 에 XXXL/4XL/5XL(scaleEm 1.5/1.75/2.0) 추가 (index 기반 atMax 가 새 천장 5XL 에서 Aa+ disable 자동 처리), `e2e/settings.spec.ts` 9단계 round-trip(xs→x5l) + indicator(5XL 200%) 갱신, `src/lib/__tests__/settings.test.ts` 신규 3키 migrate 보존 가드. SW: 라우트/링크/Content-Type/자산경로 변경 0 → v40 유지 (bump 불요, GOAL #34 와 동일 shape). **#56 (dvo-dev-cl) — 본 docs WI**: PRD §12.1 FR-165 + traceability-matrix + traceability-auto 를 9단계/200% 로 현행화. | UI / 접근성 | P2 | **GOAL #34 6단계(천장 137.5%) 도입 완료 → GOAL #50 이 9단계(천장 200%)로 확장 완료 (#52 merged `26fec4f`, docs #56). SW v40 유지 (bump 불요).** |
| FR-167 | **Gospel Canticle 본문 절 구분 — PDF 대문자-시작 규칙 (시편 FR-161 동일 규칙 차용, GOAL #35)**: 복음 성가(Benedictus/Magnificat/Nunc Dimittis)의 `src/data/loth/ordinarium/canticles.json` `verses[]` 가 시편 본문과 달리 PDF 의 'Cyrillic 대문자 시작 = 새 절' 규칙(시편 `regroupPhrasesByCapitalStart`, `scripts/build-phrases-into-rich.mjs:172,194-223`, 정규식 `/^[А-ЯЁӨҮ]/`)을 통과하지 않아 일관성 없이 병합·분리돼 있던 결함(WI #36 진단)을 교정. 사용자 보고(2026-05-26): 'pdf 에서 대문자 시작하는 것으로 절구분 하는 것이 구현이 안 되는 거.' **범위 = 절 구분만**(사용자 결정 — 렌더러/타입/resolver 무변경, 시각 단락 spacing 불변). `scripts/extract-gospel-canticles.mjs` 가 `parsed_data/full_pdf.txt` 본문 라인(Benedictus 1072-1110 / Magnificat 1303-1323 / Nunc Dimittis 17847-17854)에서 시편 `regroupPhrasesByCapitalStart` 를 **재사용(import)** 해 verses[] 재생성 — 각 대문자-시작 라인=새 절, 소문자/숫자/따옴표 시작 라인=직전 절에 공백 join(wrap-continuation 흡수). 결과: benedictus 19→25 / magnificat 12→19 / nuncDimittis 4→4(이미 정합·불변). `paragraphBoundaries` 는 현행 시각 단락 구조를 텍스트 앵커로 무손실 매핑(benedictus `[1,2,3,4,5,8,12,14,16]`→`[1,3,5,7,9,12,15,18,21]`, magnificat `[4,6,8,9]`→`[5,7,11,13]`, nunc `[1,2]` 불변) — 단락 grouping 불변, 단락 내부 절 분할만 교정(scope='절 구분만' 충실). 렌더는 `src/components/prayer-sections/gospel-canticle-section.tsx` 의 기존 `verses[]`→`<p data-role="gospel-canticle-verse">` 경로 그대로(무변경). **검증**: 단위 `gospel-canticle-section.test.ts` (capital-start 불변식 + verse 개수 + 분리/흡수 케이스 + 새 paragraphBoundaries), e2e `gospel-canticle-verse-division.spec.ts` (lauds Benedictus 25 / vespers Magnificat 19 절 + 분리 문구), 데이터 정합 가드 = NFR-009l. **CACHE_VERSION**: canticles.json = 번들 import JSON 데이터 — 자산 경로/SW 로직/precache/Content-Type 변경 0 → bump 불요(`public/sw.js` 무관). | 데이터 / 콘텐츠 | P2 | WI #37 완료 (데이터 재분절 + 추출/verify 스크립트 + 테스트) |
...
| NFR-009f | **Refrain 정합성 — 양방향 게이트 (denylist + allowlist)**: rich catalog 빌드 (`scripts/build-psalter-texts-rich.mjs`) 가 두 JSON (`refrain-denylist.json` + `refrain-allowlist.json`) 을 모두 consult 한 후에만 `psalter-texts.rich.json` 을 갱신해야 한다 (FR-160). 양방향 게이트의 의미: (1) denylist 차단 (NFR-009e 의 FP 0 보장) + (2) allowlist 보강 (FN 보강 — threshold=3 미달 authentic refrain 의 forced 마킹). allowlist entry 추가 조건: (a) PDF 원문 페이지 + 본문 색상 시각 확인 (b) GILH/공식 전례 근거 (`liturgical_basis` 필드 mandatory — 단순 lexical 반복이 아닌 회중 응답 기능이 입증되어야 함). denylist ∩ allowlist (논리 모순) 시 denylist 우선 — 알려진 false-positive 가 allowlist 추가로 silently 재도입되는 회귀 방지. NFR-009e (FP 차단) 와 본 NFR-009f (양방향) 는 보완관계 — 둘 다 만족해야 데이터 레벨 refrain 의미 (회중 응답 식별) 가 정합. **참고 (2026-05-14 정책 정정)**: 시편 본문 까만색 통일 정책으로 refrain 라인은 UI 에서 빨간색으로 가시화되지 않는다. 본 NFR 의 의미는 시편 본문 안에서는 "data-role 메타데이터 = 회중 응답 식별 / e2e selector / 향후 보이스 강조 등 비-색상 기능" 으로 좁혀지며, §12.1 루브릭 규칙의 "빨간색 = 회중 응답" semantics 는 시편 본문 외 (intercessions dash, responsory 응답, dismissal 등 FR-126/FR-150/FR-152) 영역에서만 유효하다. | 완료 |
| NFR-009e | **Refrain mistag 0 보장 (denylist 게이트)**: rich catalog 빌드 (`scripts/build-psalter-texts-rich.mjs`) 가 `src/data/loth/refrain-denylist.json` 을 consult 한 후에만 `psalter-texts.rich.json` 을 갱신해야 한다 (FR-160). denylisted ref 는 `detectRefrainLines` 가 빈 Set 을 반환해 어떤 line 도 `role='refrain'` 으로 태그되지 않으며, 결과적으로 UI `<span data-role="psalm-stanza-refrain">` 가 부여되지 않음을 데이터 레벨에서 보장한다. denylist entry 추가 조건: PDF 원문 페이지 + 본문 색상 (검정 vs rubric red) 시각 확인 필수 (`evidence_pdf` 필드 기재). 잘못된 refrain mistag 은 §12.1 루브릭 규칙 (FR-126/FR-128/FR-150/FR-152) 의 "빨간색 = 전례 지시문/공동 응답" semantics 및 refrain 데이터 의미를 오염시키므로, "본문 부재" 보다 사용자 신뢰에 더 큰 손상이다 (NFR-009b 와 동일 정신). **참고 (2026-05-14 정책 정정)**: 시편 본문 까만색 통일 정책 적용으로 refrain 라인의 visual 색상 트리거 (`text-red-700`) 는 제거됨 — 본 게이트가 보호하는 mistag 손상은 (a) 데이터 모델 (`role='refrain'` 의미) 정합 + (b) e2e selector / 회중 응답 식별 안정성 영역으로 좁혀짐. UI 빨간글씨 노출 0 보장은 더 이상 본 NFR 의 직접 contract 가 아니다 (시편 본문 전체가 까만색이므로 자동 충족). | 완료 |
...
| NFR-009j | **Phrase coverage 정합성 (FR-161 R-6)**: `src/data/loth/prayers/commons/psalter-texts.rich.json` 의 `kind:'stanza'` 블록에 `phrases?: PhraseGroup[]` (R-2 builder 가 주입) 가 존재하면, 해당 stanza 의 `lines[]` 모든 인덱스를 (1) PhraseGroupSchema (R-3 SSOT, src/lib/schemas.ts) schema 통과, (2) `0 ≤ start ≤ end < lines.length` bounds, (3) 두 PhraseGroup 의 lineRange 비-overlap, (4) 정렬 후 lineRange 가 `[0, lines.length)` 전 인덱스를 gap 없이 cover — 의 4 invariant 를 동시에 만족해야 한다. phrases 가 부재한 stanza 는 검사 대상이 아니다 (R-2 의 additive 계약 — 미주석 stanza 는 legacy line-by-line 렌더 유지). 검증 스크립트: `node scripts/verify-phrase-coverage.js [--check] [--ref <key>] [--target <path>]` — 위반 0 시 exit 0, 1+ 시 exit 1. CI gate 후보. | 완료 |
| NFR-009k | **찬가 제목↔본문↔페이지 정합 (contamination 재발 가드)**: `src/data/loth/ordinarium/hymns.json` 의 hymn 본문은 `parsed_data/full_pdf.txt` (성무일도 찬가집, 진실원천) 의 `<N>. <title>` body-region 헤더(hymns-index title 대조) 기준 순차 재추출 결과와 정합해야 한다. GOAL #32 배경: 상류 `divine-office-reader` PDF 파서의 hymn-경계 segmentation 실패로 122개 중 29개가 오염(완전오염 7 / 과포획 17 / 잘림 4 + 미세잘림 #75) — 최초 import 커밋 9f1c03e 부터. WI #34 가 `scripts/extract-hymns-from-pdf.ts` 로 `parseHymn` 재사용 + 정확 경계 + 페이지경계 run 제거(page-break stanza orphan 방지)로 전수 재추출. **불변식**: (1) 모든 hymn body 비어있지 않음 + numeric page, (2) 어떤 hymn 본문도 다른 hymn 의 헤더줄 `M. <title_M>`(M≠N, 과포획 signature)을 포함하지 않음, (3) full_pdf.txt 재추출본과 whitespace-normalized 본문/page 일치. **검증**: (a) CI — `src/lib/__tests__/hymn-contamination.test.ts` (committed hymns.json 만 사용, full_pdf.txt 불요) + e2e `e2e/hymn-contamination.spec.ts` (2026-05-25 lauds 렌더 = 진짜 #3 'Ааваа миний Ааваа'); (b) 로컬 전수 — `node --experimental-strip-types scripts/extract-hymns-from-pdf.ts --verify` (full_pdf.txt 필요, drift>0 시 exit 3). rich.json 은 `build-hymns-rich.mjs` + `build-hymn-phrases-into-rich.mjs --all` 2-step 재생성 (phrase 데이터 보존). | 완료 |
| NFR-009l | **복음 성가 본문↔PDF 대문자-절 정합 (capital-start 재분절 회귀 가드, FR-167 / GOAL #35)**: `src/data/loth/ordinarium/canticles.json` 의 benedictus/magnificat/nuncDimittis `verses[]` 는 `parsed_data/full_pdf.txt` (성무일도 4주 시편집, 진실원천) 본문 라인을 시편 `regroupPhrasesByCapitalStart` (`scripts/build-phrases-into-rich.mjs:172,194-223`, `/^[А-ЯЁӨҮ]/`) 로 재분절한 결과와 정합해야 한다. WI #36 진단 배경: canticle verses[] 가 시편 phrase 파이프라인을 통과하지 않아 PDF 대문자-시작 시행 구조와 무관하게 일관성 없이 병합·분리돼 있었음(예: magnificat verse[3] 가 PDF 두 대문자 시행 병합, benedictus 소문자 wrap-continuation 이 별도 절로 분리). **불변식**: (1) 모든 verse 의 첫 글자가 Cyrillic 대문자(소문자/숫자/따옴표 시작 wrap-continuation 이 직전 절에 흡수됨), (2) full_pdf.txt 재추출 verses[] 와 일치(benedictus 25 / magnificat 19 / nuncDimittis 4), (3) paragraphBoundaries 가 현행 시각 단락 구조를 새 verses[] 인덱스로 무손실 보존(단락 spacing 불변). **검증**: (a) CI — `src/components/prayer-sections/__tests__/gospel-canticle-section.test.ts` (committed canticles.json 만 사용, full_pdf.txt 불요 — 대문자 불변식 + verse 개수 + 분리/흡수 마커 + 새 paragraphBoundaries) + e2e `e2e/gospel-canticle-verse-division.spec.ts` (lauds Benedictus 25 / vespers Magnificat 19 절 렌더 + 분리 문구); (b) 로컬 전수 — `node scripts/extract-gospel-canticles.mjs --verify` (full_pdf.txt 필요, drift>0 시 exit 1; git worktree 는 gitignore 된 full_pdf.txt 부재이므로 `--pdf <메인 체크아웃 절대경로>` 명시). | 완료 |
