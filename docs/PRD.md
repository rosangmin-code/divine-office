...
| FR-160 | **Refrain false-positive denylist (rubric 정합화)**: `detectRefrainLines` (rich-builder.mjs Layer F) 의 threshold-based auto-detection (≥3 회 반복 line → `role='refrain'`) 이 검정 본문 (verse-ending 반복 구절, anaphoric verse-opening 등) 을 rubric refrain 으로 잘못 태그하는 케이스를 차단. **데이터**: `src/data/loth/refrain-denylist.json` (`{entries: [{ref, reason, evidence_pdf, classified_at}]}`) — denylisted ref 는 어떤 line 도 refrain 태그 안 함. **코드**: `detectRefrainLines(stanzas, {ref, denylist})` + `buildPsalterStanzasRich({stanzas, ref, denylist})` 가 denylist 우선 게이트로 빈 Set 반환 (threshold 로직 bypass). `scripts/build-psalter-texts-rich.mjs` 가 `src/data/loth/refrain-denylist.json` 을 빌드 시 1회 로드해 빌더로 propagate, 카탈로그 `psalter-texts.rich.json` 에 false-positive 가 미기록되도록 함. **A1 (task #103, 완료)**: 사용자 보고 (2026-04-26 Easter Wk4 SUN Lauds psalmody[2] Psalm 150:1-6 'Түүнийг магтагтун!' 6 reps 가 빨간글씨 노출) fast-track. **denylist 2 entries** — (a) Psalm 150:1-6 (verse-ending 6-rep 검정 본문), (b) Psalm 29:1-10 (anaphoric verse-opening 'ЭЗЭНий дуу хоолой' 3-rep, theophany 도입 — A2 gold dataset 회신으로 추가 확정, p.81). 결과: PDF 본문 검정 충실, Daniel 3:52-57 / Daniel 3:57-88, 56 / Revelation 19:1-7 / Psalm 136:1-9 등 진정 refrain 영향 없음. **A2 (task #104, 회신 완료)**: 137 refs 전수 audit 의 gold dataset → A1 denylist 보강 입력. 추가 후보 (Psalm 116:10-19 2-rep refrain FN audit 등) 는 후속 PR 로 분리. **A3/D (별건)**: psalter-texts.json key dedup, FN audit. parser regex 무수정, 데이터 측 denylist 만으로 정합화 (NFR-009e 게이트). | 데이터 / 파이프라인 / UI | P1 | A1 완료, A2 진행 중 |
