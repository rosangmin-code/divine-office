...
| FR-155 | **부활 시기 seasonal antiphon 표시 복원 (GILH §113/§272)**: 부활 시기 동안 시편·복음 교송(Benedictus/Magnificat) antiphon 에 "Аллэлуяа" 종결이 필수. 4주 psalter 의 default 후렴은 Ordinary Time 기준으로 저작되어 있어 그대로 렌더하면 부활 시기에도 OT 형태로 노출됨. **Phase 0 (task #12, 완료, commit 52ffd37→main 92f234c)**: `src/lib/hours/seasonal-antiphon.ts` 의 `applySeasonalAntiphon(antiphon, season)` 헬퍼로 EASTER 시 append-Alleluia 보정 (idempotent + 빈값 보호 + 시즌 게이트). `resolvePsalm` + `loth-service` Layer 5 gospelCanticleAntiphon 보강 경로. **Phase 1 (task #13, 완료)**: `PsalmEntry.seasonal_antiphons?: { easter?, adventDec17_23?, lent?, christmas? }` 스키마 (`types.ts` + `schemas.ts`), resolver 체인을 `override > seasonal_antiphons[season] > default_antiphon` 로 확장, `adventDec17_23` 은 12월 17-23 날짜 게이트로 활성, PDF variant 가 사용되면 `applySeasonalAntiphon` append-Alleluia 는 SKIP (PDF 가 이미 Alleluia 포함). unit `seasonal-antiphon.test.ts` (17) + resolver `resolvers/__tests__/psalm.test.ts` (5) 추가. **Phase 2 (task #14, 대기)**: PDF 각 시편 엔트리의 rubric variant (`Амилалтын улирал:` / `12 сарын 17-23:` 등) 를 추출해 `week-*.json` 의 `seasonal_antiphons` 필드 주입. | 데이터 / 파이프라인 | P1 | Phase 0/1 완료 · Phase 2 대기 |
