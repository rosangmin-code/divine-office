...
| FR-019 | 설정 시스템 기반 | FR-019 | [PRD §7](../PRD.md#7-pdf-페이지-참조-기능) | `src/lib/settings.tsx` — Context + localStorage, `hydrated` 상태로 SSR 불일치 방지, `fontSize`/`fontFamily`/`theme` 필드 확장<br>`src/app/layout.tsx` — `<SettingsProvider>` 래핑, pre-paint 인라인 스크립트로 FOUC 방지<br>`src/app/pray/[date]/[hour]/page.tsx` — `<SettingsToggle />` 배치<br>`src/app/settings/page.tsx` — 통합 설정 페이지 | `e2e/page-references.spec.ts` — 테스트 #2,5<br>`e2e/settings.spec.ts` — 설정 페이지 전체 | 완료 |
