...
| B3 (2026-09-14, requested as "STC-005") | `өргөнөөсөө` → `өргөөнөөсөө` | `propers/christmas.json:18` (`weeks.dec25.SUN.vespers.gospelCanticleAntiphon`, Christmas First Vespers Magnificat antiphon, book p.586-587) | **Printed-page render** (physical p.294 / book p.587 @200dpi, scratchpad `pd3/p294-right-top.png`) reads `өргөөнөөсөө` — the render is the primary evidence (memory rule *PDF 판정은 인쇄면 렌더로*). E1✓ `өргөнөөсөө` non-word in slot (`өргөн` "wide" takes no `-нөөсөө`) · E2✓ one dropped `ө` · E3✓ `өргөө-нөөс-өө` ("from his chamber", ablative+reflexive of `өргөө`, Ps 19:6) · E4✓ `өргөөнөөс` 28× in `src/data`, three-`ө` form 1× (this cell) | **Data-entry** — `parsed_data/full_pdf.txt:20293` has `өргөөнөөсөө`; `sanctoral/solemnities.json:879` (12-25 firstVespers copy) identical; three-`ө` form **0×** in the PDF | **CORRECT** — fidelity restore (PDF already correct → no ledger row, same class as B1/B2; `source-typo-corrections.test.ts` guards this). Applied 2026-09-14 (product decision §6-4 of `docs/bug-reports/2026-09-14-eve-vespers-alternate-and-rich.md`); `goal210-hymn-pagebreak-merge.test.mjs` christmas.json hash re-pinned. Effect before: `applyRichSourceParity` saw the seasonal dec25 cell and the sanctoral 12-25 firstVespers plain as different antiphons. |
