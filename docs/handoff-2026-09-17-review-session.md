...
- **후속 (2026-09-17)**: `c6e826b` CI Node 22 · `34cedc4` FR-174 404 상태코드 · `76f783c` FR-175 성토요일/토요일 성탄 저녁기도
...
| 중 | 2028-01-01 → 공현 제1저녁기도 미승격 | 책이 공현 EP I 을 별도 블록으로 인쇄하지 않음 → **데이터 설계 결정 필요**. FR-175 에서 이 날의 카드↔본문 불일치는 그대로 남겨 뒀다 |
| 중 | 평일 대축일 다음날이 평범한 주일 | 2026-08-15 토요일 성모 승천 등 — 순위표상 I.3 > II.6 이나 앱은 주일 제1저녁기도로 넘긴다. 카드·본문이 서로 일치하므로 버그가 아니라 **책 의도 판단 필요** (FR-175 리포트 §6) |
