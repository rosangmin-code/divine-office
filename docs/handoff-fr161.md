# FR-161 Handoff (2026-04-29, R-17 land 후 갱신)
...
origin/main 동기화 완료 (`e6b5b15` push 완료, Vercel 재배포 트리거됨).

## 검증 baseline (main, 2026-04-29 R-17 land 직후)

| 항목 | 값 |
|---|---|
| vitest | **668 PASS** / 0 FAIL (R-16 663 + R-17 신규 5 case) |
| tsc | 0 errors |
...
**multi-block flow (R-17 land 완료 #198)**: `flow="natural"` 와 `flow="sentence"` 둘 다 entire `PrayerText` 레벨에서 처리. block 경계 hard break 0. 사용자 reported 페이지 458 시편 마침 기도문 "ариун нэр" / "гай зовлон" 단어 사이 줄바꿈 → fix 적용. land 후 단독 검증 완료 (vitest 668 / tsc 0 / verify-phrase-coverage 215/0). `e6b5b15` Merge 198-member-01 push, Vercel 재배포 진행.
...
- `flattenForSentenceFlow(content)` — 모든 blocks 를 unified line list 로 평탄화 후 R-16 sentence detection 적용
...
### 1. 사용자 모바일 시각 검증 (R-17 결과 확인) — 권고

- iOS Safari / Android Chrome — Vercel 재배포 (e6b5b15) 적용 후 페이지 458 시편 마침 기도문 확인
  - "ариун нэр" / "гай зовлон" 단어 사이 hard break 없음 확인
  - 기도문 전체 한 단위로 viewport-driven wrap
- 짧은 독서 (`flow="natural"` caller) — multi-block hard break 0 확인
- 전체 마침 기도문 (`flow="sentence"` caller) — 두 문장 visible 분리 + 각 자연 wrap 확인
